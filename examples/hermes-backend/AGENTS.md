...
Hermes has no automatic observation path in this integration. Use `cogmem_episode_append` for one message or bounded `cogmem_episode_import` for a session batch after meaningful conversation, and call `cogmem_recall` before memory-dependent answers. Supply stable external message IDs so retries are idempotent. These tools write Raw Ledger and episode control metadata only; they do not run Dream or create durable beliefs. Use `cogmem_episode_status` to inspect boundaries and lag, `cogmem_episode_seal` for an explicit boundary, and MCP `cogmem_dream_tick` with `maintenanceMode: true` only as an operator/host-owned background action. Then run normal governance.
