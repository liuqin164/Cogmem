...
`raw_then_dream` stores full raw evidence first and defers semantic compilation. `KernelAgentMemoryBackend` then applies bounded CPU relation rules to group live session events into episodes. Existing OpenClaw/Hermes importers use the same assembler and seal explicit import batches. Assistant completion and idle timeout are soft boundaries; explicit user closure, manual seal, and import batch boundaries can hard-seal. A soft-sealed episode may reopen inside the same project/session and return window. A hard-sealed episode never reopens automatically.

`MemoryKernel.runDreamTick({ projectId, mode: "auto" })` processes only eligible sealed episode jobs. It atomically leases bounded work, recovers expired leases, records a run receipt, and never deletes raw events. Auto mode selects no work, micro, normal, or deep processing from backlog state. `MemoryKernel.runDreamCurator()` remains a low-level compatibility/repair API; normal host scheduling should use `runDreamTick()`.

The built-in curator is deterministic and local-first. It suppresses operational noise and reads one sealed episode at a time. Assistant self-correction is conversational context, not a user-owned correction or contradiction. Negative-form questions such as `是不是...` do not create correction records. Every candidate carries `sourceEpisodeId`, exact raw `eventId` evidence, role, chronological fields, and `sourceAnchor`. Episode summaries, closure receipts, `COGMEM_*` blocks, activation receipts, and strategy outcomes cannot satisfy evidence requirements.
