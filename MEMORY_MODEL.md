...
Existing OpenClaw/Hermes importers use the same assembler. High-confidence import groups may batch-seal; low-confidence groups soft-seal for review unless an operator explicitly forces sealing. Cross-domain topic switches hard-seal, same-project subtopic shifts remain in the episode, and ambiguous shifts create a soft link. A soft-sealed episode may reopen inside the same project/session/source/thread return window. A hard-sealed episode never reopens automatically.

`MemoryKernel.runDreamTick({ projectId, mode: "auto" })` processes only eligible sealed episode jobs. It atomically leases bounded work, records a run receipt, and never deletes raw events. Auto mode selects no work, micro, normal, or deep processing from backlog state. Micro is a single bounded episode with at most 20 candidates, normal uses a small batch with at most 100 candidates per curator call, and deep permits cross-episode consistency review with at most 500 candidates. Retryable failures receive `retryAfter`; terminal evidence/schema failures are never automatically reclaimed. Manual retry requeues only retryable jobs and preserves attempt counts.
...
An explicit assistant proposal followed by a distinct accepting user event may use paired `assistant_proposal_confirmed_by_user` evidence. A proposal without later unambiguous user confirmation cannot become user-owned memory. Correction candidates must identify a corrected claim/target or remain `needs_confirmation`. Decision episodes may propose temporal decision candidates with occurrence and sequence bounds. Prospective candidates continue through `ProspectiveMemoryService`, which owns canonical deduplication and confirmation state.
...
`MemoryKernel.buildMemoryMap()` and `cogmem memory map` expose a static self-map: anatomy, data lanes, hard bounds, counters, and commands. The map includes episode counts, unassigned raw evidence, and the sealed Episode Dream backlog. Binding, governance, entity-resolution, Belief Graph, Temporal Memory, Context Cortex receipt, Prospective Memory, Strategy Cortex outcome, and Episode Dream tables are governed by core schema version 23. Migration 23 adds episode audit, semantic-summary, source/thread scope, normalized closure, and Dream retry/status metadata without rewriting Raw Ledger evidence. Use `cogmem migrate --dry-run --json` before an upgrade and `cogmem migrate --yes --backup` to apply it.
