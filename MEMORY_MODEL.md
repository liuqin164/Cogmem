...
- User-Shaped Topic Ontology: a fixed memory-class skeleton (`Time`, `Person`, `Object`, `Place`, `Project`, `Event`, `Decision`, `Goal`, `Preference`, `Correction`, `Plan`, `Evidence`, `Conversation`, `Topic`, `Entity`, `Relation`) plus project-scoped user names, aliases, paths, and relation edges. Explicit user operations are active and reversible; model proposals remain candidates.
...
- Prospective Memory: candidate-only intentions, commitments, reminders, open loops, and plans with pending, confirmed, deferred, rejected, completed, and expired states. Only explicit user confirmation can activate a due candidate. Core stores state and provenance; it never executes tasks or tools from prospective memory.
- BrainEval: release-gate metrics for recall, precision, provenance, context budget, stale and cross-project leakage, and unconfirmed prospective activation.
- Episode Store: session-scoped, auditable conversation units with open, soft-sealed, and sealed lifecycle. Episode summaries and closure receipts are control metadata, not evidence.

`forgetUser(projectId)` erases every project-owned projection introduced by these layers, not only Raw Ledger/vector state. Shared canonical entities are retained only when another project still owns an instance, and their aliases are rebuilt from the remaining instances.
...
- Episode Dream Backlog: leased, retryable jobs over sealed episodes so `raw_then_dream` does not crystallize incomplete turns or silently become unprocessed log accumulation.
...
`raw_then_dream` stores full raw evidence first and defers semantic compilation. `KernelAgentMemoryBackend` then applies bounded CPU relation rules to group live session events into episodes. Classification receives the current turn, previous user/assistant text, active episode hints, and recent relations, so short replies can confirm, reject, answer, or continue the preceding assistant contribution. Ambiguous or high-value cases may use the asynchronous hybrid review API, but its normalized result is advisory metadata only and cannot write durable memory.

Existing OpenClaw/Hermes importers use the same assembler. High-confidence import groups may batch-seal; low-confidence groups soft-seal for review unless an operator explicitly forces sealing. Topic continuity is decided from project boundary, user-defined aliases, topic/entity overlap, semantic similarity, and advisory classification—not a growing domain regex dictionary. Unknown turns become ambiguous review boundaries. A soft-sealed episode may reopen only for confirmed continuation, clarification, correction, or return-to-topic relations inside the same project/session/source/thread window; hard switches, ambiguous shifts, and noise cannot reopen it. A hard-sealed episode never reopens automatically.

`MemoryKernel.runDreamTick({ projectId, mode: "auto" })` processes only eligible sealed episode jobs. It atomically leases bounded work, records a run receipt, and never deletes raw events. Auto mode computes an effective mode per job and reports `selectedModes`; explicit mode overrides the batch. Deep is recommended for large/old backlogs, maintenance or repair work, and explicit requests. Micro is a bounded episode with at most 20 candidates, normal allows at most 100 candidates per curator call, and deep permits up to 500. `failedEpisodes` and the persisted run receipt contain episode ID, category, error, and optional `retryAfter`; terminal evidence/schema failures are never automatically reclaimed.

The built-in curator is deterministic and local-first. It suppresses operational noise and reads sealed episode input according to the selected mode. Assistant self-correction is conversational context, not a user-owned correction or contradiction. Negative-form questions such as `是不是...` do not create correction records. Every candidate carries `sourceEpisodeId`, a non-empty subset of exact raw `eventId` evidence from that episode, role, chronological fields, and `sourceAnchor`. Episode semantic summaries can expose user position, assistant contribution, decision/correction hints, and open questions, but summaries, closure receipts, `COGMEM_*` blocks, activation receipts, and strategy outcomes cannot satisfy evidence requirements.

An explicit assistant proposal followed by a later distinct accepting user event in the same session/thread may use paired `assistant_proposal_confirmed_by_user` evidence. The resolver scans a bounded prior window and stops at an intervening rejection or correction. A proposal without unambiguous user confirmation cannot become user-owned memory. `CorrectionResolver` searches active beliefs by project and claim/topic cues; an orphan or ambiguous correction remains `needs_confirmation` with an audit reason. An assistant-only decision also remains `needs_confirmation` and cannot enter normal durable temporal promotion. Prospective candidates continue through `ProspectiveMemoryService`, which owns canonical deduplication and confirmation state.

When `[memory_model]` is configured with an OpenAI-compatible chat endpoint, the same worker can call the configured memory model after deterministic extraction. This supports both local Ollama (`base_url = "http://localhost:11434/v1"`) and cloud OpenAI-compatible APIs. The model may propose user preferences, project memories, long-term goals, prohibitions/boundaries, failure lessons, diagnostic conclusions, session summaries, topic summaries, temporal fact updates, conflicts, semantic tags, indexing decisions, semantic relations, and edge adjustments. LLM output is normalized into candidate records only. Provider `projectId` never overrides CPU-owned scope. Episode-mode candidates must cite a non-empty exact subset of current raw event IDs; `evidenceEventIds: ["all"]`, missing IDs, hallucinated IDs, and fallback-to-first-events behavior are rejected. Semantic summaries are always `hint_only_not_evidence`; mixed user/assistant summaries are session hints, not user-owned durable beliefs. CPU governance still owns status changes such as `candidate`, `needs_confirmation`, `promoted`, `superseded`, and `rejected`.
...
```bash
cogmem episode status --project <project> --json
cogmem dream tick --project <project> --mode auto --json
...
- Dreaming-style consolidation: implemented as a candidate-only curator that proposes categorized candidates and summaries from sealed episodes backed by exact raw event IDs. CPU governance must decide promotion and every candidate must retain source refs.
