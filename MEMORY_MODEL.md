...
`MemoryKernel.buildMemoryMap()` and `cogmem memory map` expose a static self-map: anatomy, data lanes, hard bounds, counters, and commands. The memory binding counters include bindings, topics, entities, clusters, and edges. Imported or adapter-written raw events can be attached with `cogmem memory bind --project <id> --json`; maintenance tick will suggest this command when high-value raw user events are still unbound. Binding, governance, and entity-resolution lifecycle tables are governed by core schema version 16. Use `cogmem migrate --dry-run --json` before an upgrade and `cogmem migrate --yes --backup` to apply pending migrations without rewriting Raw Ledger evidence. Agents should use the map to understand how to operate the memory kernel, not as a replacement for governed recall.
