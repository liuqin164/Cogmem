...
export function createMemoryUsageReceipt(input: CreateMemoryUsageReceiptInput): MemoryUsageReceipt {
...
export function formatMemoryUsageBridge(receipt: MemoryUsageReceipt, maxChars = 1200): string {
  const lines = [
...
export function shouldInjectMemoryUsageBridge(query: string, receipt: MemoryUsageReceipt): boolean {
...
function extractThemes(items: Array<Pick<AgentRecallItem, 'text' | 'tags'>>): string[] {
  const themes: string[] = [];
  for (const item of items) {
    const tagTheme = item.tags.find((tag) => tag.startsWith('topic:') || tag.startsWith('collection:'));
    if (tagTheme) themes.push(tagTheme);
    const sentence = firstSentence(item.text, 140);
    if (sentence) themes.push(sentence);
  }
  return uniqueNonEmpty(themes);
}

function firstSentence(text: string, limit: number): string | undefined {
...
function formatSourceAnchor(anchor: MemoryUsageReceiptSourceAnchor): string {
  return [
