...
export class MemoryGovernanceValidator {
  constructor(private readonly findEvidence: GovernanceEvidenceLookup) {}

  validate(plan: MemoryGovernancePlan): MemoryGovernanceValidationResult {
    const issues: MemoryGovernanceIssue[] = [];
    const operationIds = new Set<string>();
    const idempotencyKeys = new Set<string>();

    if (!plan.planId.trim()) issues.push({ code: 'missing_plan_id', message: 'planId is required.' });
    if (plan.operations.length === 0) issues.push({ code: 'empty_plan', message: 'At least one operation is required.' });
