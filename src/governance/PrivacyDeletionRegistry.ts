...
export const PRIVACY_BASELINE_CLASSIFICATION: Readonly<Record<string, PrivacyTableClassification>> = Object.freeze(Object.fromEntries(
  V0_5_BASELINE_TABLES.map((table) => [table, SHARED_CANONICAL.has(table) ? 'shared_canonical' : OPERATIONAL_NON_PERSONAL.has(table) ? 'operational_non_personal' : 'project_owned']),
