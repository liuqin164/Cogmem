...
export function validateMemoryFrame(value: unknown, options: { allowEmptyEvidence?: boolean } = {}): MemoryFrameValidationResult {
  ...
  if (malformedNode || malformedRelation || malformedTime || malformedState) return { valid: false, errors: ['invalid_memory_frame_nested_shape'] };
  const limits = MEMORY_FRAME_LIMITS;
  const idLimit = limits.id;
  const invalidEvidence = (ids: unknown[]): boolean => ids.length > limits.evidence || ids.some((id) => typeof id !== 'string' || !id.trim() || id.length > idLimit);
  ...
  if (frame.primaryLanguage !== undefined && (typeof frame.primaryLanguage !== 'string' || frame.primaryLanguage.length > limits.language)) errors.push('invalid_primary_language');
  ...
  if (invalidEvidence(frame.evidenceEventIds)) errors.push('frame_evidence_limit_exceeded');
  ...
  for (const dimension of MEMORY_FRAME_REQUIRED_DIMENSIONS) {
    if (!frame.nodes.some((node) => node.dimension === dimension && node.label.trim())) errors.push(`${dimension}_node_required`);
  }
  for (const node of frame.nodes) {
    if (typeof node.frameNodeId !== 'string' || !node.frameNodeId.trim() || node.frameNodeId.length > idLimit || typeof node.label !== 'string' || !Array.isArray(node.evidenceEventIds)) { errors.push('invalid_frame_node_shape'); continue; }
    if (!node.label.trim()) errors.push(`empty_frame_node_label:${node.frameNodeId}`);
    if (node.label.length > limits.text || String(node.dimension).length > limits.dimension) errors.push(`frame_node_text_limit_exceeded:${node.frameNodeId}`);
    ...
    if ((!node.evidenceEventIds.length && !options.allowEmptyEvidence) || invalidEvidence(node.evidenceEventIds)) errors.push(`node_evidence_required:${node.frameNodeId}`);
    if (!node.evidenceEventIds.every((id) => evidence.has(id))) errors.push(`node_evidence_not_in_frame:${node.frameNodeId}`);
    if (node.description !== undefined && node.description.length > limits.text) errors.push(`node_description_limit_exceeded:${node.frameNodeId}`);
    if (node.aliases !== undefined && (!Array.isArray(node.aliases) || node.aliases.length > limits.aliases || node.aliases.some((alias) => typeof alias !== 'string' || alias.length > limits.alias))) errors.push(`invalid_node_aliases:${node.frameNodeId}`);
    if (Array.isArray(node.aliases) && node.aliases.some((alias) => !alias.trim())) errors.push(`empty_node_alias:${node.frameNodeId}`);
    if (node.canonicalHint?.nodeId && !['episode', 'project', 'raw_event'].includes(node.dimension)) errors.push(`mutable_identity_hint_not_allowed:${node.frameNodeId}`);
    if (node.canonicalHint?.nodeId && ['episode', 'project', 'raw_event'].includes(node.dimension)) {
      ...
      if (!expected || node.canonicalHint.nodeId !== expected) errors.push(`immutable_identity_hint_mismatch:${node.frameNodeId}`);
    }
  }
  for (const relation of frame.relations) {
    ...
    if (!relation.sourceFrameNodeId.trim() || !relation.targetFrameNodeId.trim() || !relation.relationType.trim() || relation.sourceFrameNodeId.length > idLimit || relation.targetFrameNodeId.length > idLimit || relation.relationType.length > idLimit) errors.push('invalid_relation_identity');
    ...
    if (!Array.isArray(relation.evidenceEventIds) || (!relation.evidenceEventIds.length && !options.allowEmptyEvidence) || invalidEvidence(relation.evidenceEventIds)) errors.push('relation_evidence_required');
    ...
    if ((relation.validFrom !== undefined && !Number.isFinite(relation.validFrom)) || (relation.validTo !== undefined && !Number.isFinite(relation.validTo))) errors.push('relation_invalid_time');
  ...
  for (const transition of frame.stateTransitions) {
    ...
    if (typeof transition.to !== 'string' || (transition.from !== undefined && typeof transition.from !== 'string') || !Array.isArray(transition.evidenceEventIds) || !transition.to.trim() || transition.to.length > limits.text || (transition.from !== undefined && (!transition.from.trim() || transition.from.length > limits.text)) || !Number.isFinite(transition.confidence) || transition.confidence < 0 || transition.confidence > 1 || (!transition.evidenceEventIds.length && !options.allowEmptyEvidence) || invalidEvidence(transition.evidenceEventIds)) errors.push('invalid_state_transition');
