...
export function validateMemoryFrame(value: unknown): MemoryFrameValidationResult {
  ...
  for (const reference of frame.temporalReferences) {
    if (!reference.label.trim() || !Number.isFinite(reference.confidence) || reference.confidence < 0 || reference.confidence > 1) errors.push('invalid_temporal_reference');
  ...
  for (const transition of frame.stateTransitions) {
    ...
    if (!transition.to.trim() || !Number.isFinite(transition.confidence) || transition.confidence < 0 || transition.confidence > 1) errors.push('invalid_state_transition');
