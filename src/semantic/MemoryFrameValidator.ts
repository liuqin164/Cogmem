...
export function validateMemoryFrame(value: unknown, options: { allowEmptyEvidence?: boolean } = {}): MemoryFrameValidationResult {
  ...
  if (!frame.processor || typeof frame.processor.promptVersion !== 'string' || !frame.processor.promptVersion.trim() || !Number.isFinite(frame.processor.generatedAt) || (frame.processor.provider !== undefined && typeof frame.processor.provider !== 'string') || (frame.processor.model !== undefined && typeof frame.processor.model !== 'string')) errors.push('invalid_processor_metadata');
  ...
  for (const node of frame.nodes) {
    if (typeof node.frameNodeId !== 'string' || !node.frameNodeId.trim() || typeof node.label !== 'string' || !Array.isArray(node.evidenceEventIds)) { errors.push('invalid_frame_node_shape'); continue; }
    ...
    if (node.aliases !== undefined && (!Array.isArray(node.aliases) || node.aliases.length > limits.aliases || node.aliases.some((alias) => typeof alias !== 'string' || alias.length > 1000))) errors.push(`invalid_node_aliases:${node.frameNodeId}`);
