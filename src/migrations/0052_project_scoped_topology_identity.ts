      ...
      if (!resolved) {
        db.prepare(`INSERT OR REPLACE INTO topology_identity_quarantine
          ...
          VALUES(?,?,?,?,?,?,?)`).run(
            ...
            JSON.stringify(entry), 'entry_project_scope_unresolved', Date.now()
          );
        continue;
      }
...
function inferEntryScope(db: Database, entry: Entry): { scope: string; neuronId?: string } | null {
  for (const neuronId of [entry.neuron_id, neuronForFact(db, entry.fact_id), neuronForEvent(db, entry.event_id), neuronForBelief(db, entry.belief_id)]) {
    ...
    if (scope !== null) return { scope, neuronId: neuronId ?? undefined };
  }
  if (entry.unit_id && tableExists(db, 'interaction_units')) {
    ...
    const scopes = new Set(ids.map((id) => scopeForNeuron(db, id)).filter((value): value is string => value !== null));
    if (scopes.size === 1) return { scope: [...scopes][0]!, neuronId: ids[0] };
  }
  return null;
}
...
function scopeForNeuron(db: Database, id: string | null): string | null {
  ...
  const row = db.prepare(`SELECT COALESCE(project_id,'') AS scope FROM neurons WHERE id=?`).get(id) as { scope?: string } | null;
...
function normalizeDreamLedgerKeys(db: Database): void {
  ...
  for (const row of rows) {
    ...
    if (scope === null && row.project_key === '__global__') db.prepare(`INSERT OR IGNORE INTO dream_ledger_state(project_key,project_id,last_dreamed_global_seq,last_dreamed_at,updated_at) VALUES('scope:0:','',?,?,?)`).run(row.last_dreamed_global_seq, row.last_dreamed_at, row.updated_at);
  }
}
