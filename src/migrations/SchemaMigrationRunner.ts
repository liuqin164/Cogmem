...
export class SchemaMigrationRunner {
  ...
  private assertSupportedReleaseSchema(): void {
    ...
    if (latest && latest < '0031') throw new Error(`unsupported_release_schema:${latest}`);
    if (legacyVersion !== undefined && legacyVersion > 32) throw new Error(`unsupported_development_schema:${legacyVersion}`);
  }
