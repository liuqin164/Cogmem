...
export const migration_0030: Migration = {
  ...
  up(db) {
    if (db.prepare(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'memory_events'`).get()) {
      const columns = new Set((db.prepare(`PRAGMA table_info(memory_events)`).all() as Array<{ name: string }>).map((row) => row.name));
    ...
    if (db.prepare(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'memory_episode_events'`).get()) {
      ...
      `).all() as Array<{ episode_id: string; event_id: string; position: number }>;
      ...
      for (const link of links) temporary.run(-link.position - 1, link.episode_id, link.event_id);
      for (const link of links) temporary.run(positions.get(`${link.episode_id}\u0000${link.event_id}`) ?? 0, link.episode_id, link.event_id);
      ...
      const eventColumns = hasEvents
        ? new Set((db.prepare(`PRAGMA table_info(memory_events)`).all() as Array<{ name: string }>).map((row) => row.name))
        : new Set<string>();
      ...
      if (hasEpisodes) {
        ...
        for (const episode of episodes) {
          const actualCount = Number((count.get(episode.episode_id) as { count?: number }).count ?? 0);
          const firstEventId = (first.get(episode.episode_id) as { event_id?: string } | null)?.event_id ?? null;
          const lastEventId = (last.get(episode.episode_id) as { event_id?: string } | null)?.event_id ?? null;
          if (hasEvents && hasGlobalSeq) rebuild.run(actualCount, firstEventId, lastEventId, firstEventId, lastEventId, episode.episode_id);
          else rebuild.run(actualCount, firstEventId, lastEventId, episode.episode_id);
        }
      }
      const receiptTable = Boolean(db.prepare(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'episode_closure_receipts'`).get());
      if (receiptTable) {
        const receipts = db.prepare(`SELECT receipt_id, episode_id, source_event_ids_json FROM episode_closure_receipts`).all() as Array<{ receipt_id: string; episode_id: string; source_event_ids_json: string }>;
        ...
        for (const receipt of receipts) {
          ...
          try {
            ...
            eventIds = Array.isArray(parsed) ? parsed.filter((id): id is string => typeof id === 'string') : [];
          } catch {
            eventIds = [];
          }
          const positions = new Map((selectPosition.all(receipt.episode_id) as Array<{ event_id: string; position: number }>).map((row) => [row.event_id, row.position]));
