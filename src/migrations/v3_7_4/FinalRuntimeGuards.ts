...
export function installRuntimeProvenanceGuards(db: Database): void {
  installFinalTriggers(db, (name) =>
    PROVENANCE_TRIGGER_PREFIXES.some((prefix) => name.startsWith(prefix))
...
function installFinalTriggers(db: Database, matches: (name: string) => boolean): void {
  for (const trigger of FINAL_AUXILIARY_OBJECTS) {
    if (trigger.type !== 'trigger' || !matches(trigger.name) || !tableExists(db, trigger.table)) continue;
    const current = db.prepare(`
      SELECT sql FROM sqlite_master WHERE type='trigger' AND name=?
    `).get(trigger.name) as { sql?: string } | null;
    if (current?.sql && normalizeSql(current.sql) === normalizeSql(trigger.sql)) continue;
...
function provenanceDependenciesPresent(db: Database, name: string): boolean {
  const dependencies = name.startsWith('synapses_scope_') ? ['neurons']
    ...
    : name.startsWith('pending_entity_scope_') ? ['neurons']
