...
function classifyTaskRecovery(db: Database): void {
  ...
  if (!columns.has('recovery_status')) db.exec(`ALTER TABLE task_identity_restoration_manifest ADD COLUMN recovery_status TEXT NOT NULL DEFAULT 'unresolved'`);
...
function auditEntityIsolation(db: Database): void {
  ensureAuditTable(db);
  if (tableExists(db, 'entity_aliases') && tableExists(db, 'entity_instances')) {
    ...
    for (const row of invalid) quarantine(db, 'entity_alias_0057', String(row.alias_id), row, String(row.project_id ?? ''), 'entity_alias_scope_unproven');
    if (invalid.length) db.exec(`DELETE FROM entity_aliases WHERE alias_id IN (
  ...
  if (tableExists(db, 'entity_relations')) {
    ...
    for (const row of invalid) quarantine(db, 'entity_relation_0057', String(row.relation_id), row, String(row.project_id ?? ''), 'entity_relation_scope_unproven');
    if (invalid.length) db.exec(`DELETE FROM entity_relations WHERE relation_id IN (
...
function rebuildAliasConflicts(db: Database): void {
  if (!tableExists(db, 'entity_alias_conflicts') || !tableExists(db, 'entity_aliases') || !tableExists(db, 'entity_instances')) return;
  ...
  const rows = db.prepare(`SELECT a.project_id,a.normalized_alias,i.type,GROUP_CONCAT(DISTINCT a.entity_id) AS entity_ids,
    ...
    GROUP BY a.project_id,a.normalized_alias,i.type HAVING COUNT(DISTINCT a.entity_id)>1`).all() as Array<{
      project_id: string; normalized_alias: string; type: string; entity_ids: string; created_at: number; updated_at: number;
    }>;
...
function migrateIngestionIdentity(db: Database): void {
  ...
  } else createProcessedTable(db);
}

function migrateEventCounters(db: Database): void {
  ...
  const global = db.prepare(`SELECT COALESCE(MAX(global_seq),0) AS value FROM memory_events`).get() as { value: number };
  upsert.run('global', global.value);
  for (const row of db.prepare(`SELECT project_scope,stream_id,MAX(event_version) AS value FROM memory_events GROUP BY project_scope,stream_id`).all() as Array<{ project_scope: string; stream_id: string; value: number }>) {
...
export function projectIsolationCompensationSatisfied(db: Database): boolean {
  if (!tableExists(db, 'project_isolation_compensation_audit')) return false;
  if (tableExists(db, 'task_identity_restoration_manifest')) {
    if (!tableColumns(db, 'task_identity_restoration_manifest').has('recovery_status')) return false;
    if (db.prepare(`SELECT 1 FROM task_identity_restoration_manifest m
      ...
      WHERE m.recovery_status='unresolved' AND q.task_id IS NULL LIMIT 1`).get()) return false;
  }
  if (!hasColumns(db, 'ingestion_source_cursors', ['project_scope','project_id'])
    || !hasColumns(db, 'ingestion_processed_records', ['project_scope','project_id'])) return false;
  if (!tableExists(db, 'event_sequence_counters')) return false;
  if (db.prepare(`SELECT 1 FROM project_isolation_compensation_audit
    WHERE retained_count+split_count+quarantined_count+approved_deleted_count<>original_count LIMIT 1`).get()) return false;
  return true;
}

function assertProjectIsolationCompensated(db: Database): void {
  if (!projectIsolationCompensationSatisfied(db)) throw new Error('project_isolation_compensation_failed');
}

function createCursorTable(db: Database): void {
...
function createProcessedTable(db: Database): void {
...
function recordAudit(db: Database, category: string, original: number, retained: number, quarantined: number, approvedDeleted: number): void {
...
function quarantine(db: Database, type: string, id: string, row: unknown, scope: string, reason: string): void {
  if (!tableExists(db, 'topology_identity_quarantine')) throw new Error(`migration_0057_quarantine_unavailable:${type}:${id}`);
...
function count(db: Database, table: string): number { return Number((db.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get() as { count: number }).count); }
...
function hasColumns(db: Database, name: string, columns: string[]): boolean {
