...
export class DeepWritePromotionPolicy {
  ...
  evaluateAndApply(candidate: DeepWriteCandidateRecord, options: DeepWriteEvaluationOptions = {}): DeepWritePromotionDecision {
    ...
    if (!run) return this.mark(candidate, 'rejected', { outcome: 'reject', reason: 'promotion_run_missing' });
  ...
  private resolveProvenance(candidate: DeepWriteCandidateRecord): {
    projectId: string;
    neuronIds: string[];
    eventIds: string[];
  } {
    const run = this.deps.candidateStore.getRun(candidate.runId);
    if (!run) return { projectId: '', neuronIds: [], eventIds: [] };
    ...
    const addNeuron = (id: string | undefined): void => {
      if (!id) return;
      if (!hasNeurons) return;
      ...
      if (row?.scope === projectId) neuronIds.add(id);
    };
    const addEvent = (id: string | undefined): void => {
      ...
      const row = db.prepare(`SELECT COALESCE(project_id,'') AS scope,source_neuron_id FROM memory_events WHERE event_id=?`)
        .get(id) as { scope: string; source_neuron_id: string | null } | null;
      if (!row || row.scope !== projectId) return;
      eventIds.add(id);
      addNeuron(row.source_neuron_id || undefined);
    };
    for (const id of run.sourceNeuronIds) addNeuron(id);
