...
export class MemoryFrameProjector {
    ...
    for (const frame of frames) {
        ...
        for (const transition of frame.stateTransitions) {
          ...
          if (transition.from) {
            ...
            if (!existingFrom) this.atlasStore.upsertDocument({ id: fromId, projectId, nodeType: 'state', sourceId: fromId.slice(6), label: transition.from, confidence: transition.confidence, supportCount: 1, status: 'active', evidenceEventIds: transition.evidenceEventIds, metadata: { projection: 'memory_atlas.frame.v2', frameId: frame.frameId }, updatedAt: now });
  ...
  private nodeId(projectId: string, node: MemoryFrameNode, frame: MemoryFrameV1): string {
    ...
    if (aliasNodes.length > 1) throw new Error(`memory_frame_alias_ambiguous:${node.dimension}:${normalizeAlias(node.label)}`);
  ...
  private upsertEdge(projectId: string, source: string, target: string, relation: MemoryFrameRelation, frame: MemoryFrameV1, now: number): void {
    ...
    `).run(edgeId, projectId, parsedSource.type, parsedSource.id, relation.relationType, parsedTarget.type, parsedTarget.id,
      relation.confidence, 1, 0.85, 1, JSON.stringify(relation.evidenceEventIds), 'active', relation.validFrom ?? this.evidenceTime(relation.evidenceEventIds, frame.processor.generatedAt), relation.validTo ?? null, 1, 'memory_frame_projector', now, now);
