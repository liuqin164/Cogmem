...
import type { MemoryFrameNode, MemoryFrameRelation, MemoryFrameV1, StateTransition } from '../semantic/MemoryFrameTypes.js';
...
import { invalidateMemoryEdgeSupportIds, mergeMemoryEdge, mergeMemoryValidityIntervals, reduceMemoryEdges, type MemoryEdgeMergeInput, type MemoryValidityInterval } from '../binding/MemoryEdgeMerge.js';
...
interface NodeSupportSnapshot { evidenceEventIds: string[]; payload: Record<string, unknown>; payloadKey: string; confidence: number; }
interface AliasSupportSnapshot { aliasId: string; nodeId: string; alias: string; normalized: string; dimension: string; evidenceEventIds: string[]; confidence: number; }
interface EdgeSnapshot { input: MemoryEdgeMergeInput; ranges: MemoryValidityInterval[]; }
interface EvidenceOrder { occurredAt: number; globalSeq: number; }
interface CurrentStateDecision { order: EvidenceOrder; stateKeys: Set<string>; frameIds: Set<string>; }
...
export class MemoryFrameProjector {
  ...
  private rebuildUnsafe(projectId: string, now: number, options: { canonicalDocumentsRebuilt?: boolean }): MemoryFrameProjectionResult {
    ...
    const transitionOrders = new WeakMap<StateTransition, EvidenceOrder>();
    ...
    for (const frame of frames) {
        const nodeSnapshots = new Map<string, NodeSupportSnapshot>();
        const aliasSnapshots = new Map<string, AliasSupportSnapshot>();
        const edgeSnapshots = new Map<string, EdgeSnapshot>();
        if (frame.needsReview || ambiguousStateFrames.has(frame.frameId)) needsReview += 1;
        ...
        for (const node of frame.nodes) {
          ...
          for (const alias of [node.label, ...(node.aliases ?? []), ...(node.canonicalHint?.canonicalLabel ? [node.canonicalHint.canonicalLabel] : [])]) this.contributeAlias(aliasSnapshots, projectId, id, node, alias);
        }
  ...
  private contributeSupport(snapshots: Map<string, NodeSupportSnapshot>, nodeId: string, frame: MemoryFrameV1, evidenceEventIds: string[], payload: Record<string, unknown>): void {
  ...
  private flushNodeSnapshots(snapshots: Map<string, NodeSupportSnapshot>, projectId: string, frame: MemoryFrameV1, now: number): void {
  ...
  private contributeAlias(snapshots: Map<string, AliasSupportSnapshot>, projectId: string, nodeId: string, node: MemoryFrameNode, alias: string): void {
  ...
  private flushAliasSnapshots(snapshots: Map<string, AliasSupportSnapshot>, projectId: string, frame: MemoryFrameV1, now: number): void {
    for (const [, snapshot] of [...snapshots.entries()].sort(([left], [right]) => left.localeCompare(right))) this.upsertAlias(projectId, snapshot, frame, now);
  }

  private upsertAlias(projectId: string, snapshot: AliasSupportSnapshot, frame: MemoryFrameV1, now: number): void {
  ...
  private contributeEdge(snapshots: Map<string, EdgeSnapshot>, projectId: string, source: string, target: string, relation: MemoryFrameRelation, frame: MemoryFrameV1, now: number, status = 'active'): void {
  ...
  private flushEdgeSnapshots(snapshots: Map<string, EdgeSnapshot>): void {
  ...
  private resolveCurrentStates(
    frames: MemoryFrameV1[],
    nodeIdsByFrame: Map<string, Map<string, string | undefined>>,
    transitionOrders: WeakMap<StateTransition, EvidenceOrder>,
  ): Map<string, CurrentStateDecision> {
    const decisions = new Map<string, CurrentStateDecision>();
    for (const frame of frames) for (const transition of frame.stateTransitions) {
      const subjectNode = frame.nodes.find((node) => node.frameNodeId === transition.subjectFrameNodeId);
      ...
      if (comparison > 0) decisions.set(subject, { order, stateKeys: new Set([stateKey]), frameIds: new Set([frame.frameId]) });
  ...
  private reduceAffectedDocuments(projectId: string, now: number): void {
    ...
    for (const nodeId of this.affectedNodeIds) {
      ...
      const current = this.db.prepare(`SELECT label,summary,confidence,support_count,status,occurred_at,evidence_event_ids_json,metadata_json FROM memory_atlas_documents WHERE project_id=? AND node_id=?`).get(projectId, nodeId) as {
        label: string; summary?: string | null; confidence: number; support_count: number; status: string; occurred_at?: number | null; evidence_event_ids_json: string; metadata_json?: string;
      } | null;
      const supports = this.db.prepare(`SELECT support_id,payload_json,confidence,evidence_event_ids_json,source_authority,created_at FROM memory_atlas_supports WHERE project_id=? AND node_id=? AND status='active'`).all(projectId, nodeId) as Array<{ support_id: string; payload_json?: string; confidence?: number; evidence_event_ids_json?: string; source_authority?: string; created_at: number }>;
      if (!supports.length) {
      ...
      const occurredAt = parsed.map(({ payload }) => payload.occurredAt).find((value): value is number => typeof value === 'number' && Number.isFinite(value)) ?? current.occurred_at ?? null;
      ...
      if (isFrameProjection) {
        if (typeof winner.payload.frameId === 'string') metadata.frameId = winner.payload.frameId;
        if (typeof winner.payload.frameSchemaVersion === 'string') metadata.frameSchemaVersion = winner.payload.frameSchemaVersion;
      }
      ...
      if (current.support_count === supportCount && current.confidence === confidence && current.label === label
        ...
        && current.evidence_event_ids_json === evidenceJson && current.status === status && current.metadata_json === metadataJson) continue;
  ...
  private latestEvidenceOrder(eventIds: string[], fallback: number): EvidenceOrder {
    if (!eventIds.length) return { occurredAt: fallback, globalSeq: 0 };
    const row = this.db.prepare(`SELECT occurred_at,COALESCE(global_seq,0) AS global_seq FROM memory_events WHERE event_id IN (${eventIds.map(() => '?').join(',')}) ORDER BY occurred_at DESC,COALESCE(global_seq,0) DESC,event_id DESC LIMIT 1`).get(...eventIds) as { occurred_at?: number; global_seq?: number } | null;
...
function compareEvidenceOrder(left: EvidenceOrder, right: EvidenceOrder): number {
  return left.occurredAt - right.occurredAt || left.globalSeq - right.globalSeq;
}

function frameProjection(metadataJson?: string): boolean {
  try { return JSON.parse(metadataJson ?? '{}')?.projection === 'memory_atlas.frame.v2'; } catch { return false; }
}
