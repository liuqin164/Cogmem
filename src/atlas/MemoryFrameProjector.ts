...
export class MemoryFrameProjector {
  ...
  rebuild(projectId: string, now = Date.now()): MemoryFrameProjectionResult {
    ...
    for (const frame of frames) {
        ...
        for (const reference of frame.temporalReferences) {
          const dayKey = reference.occurredAt == null ? normalizeAlias(reference.label) : new Date(reference.occurredAt).toISOString().slice(0, 10);
  ...
  private nodeId(projectId: string, node: MemoryFrameNode, frame: MemoryFrameV1): string | undefined {
    ...
    if (aliasNodes.length > 1) return undefined;
