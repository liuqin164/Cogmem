  ...
  rebuild(projectId?: string): number {
    ...
    while (true) {
      ...
      for (const event of result.records) {
        if (event.role !== 'user' && event.role !== 'tool') continue;
        ...
        for (const marker of markers) {
          ...
          `).run(actionId, eventProjectId, marker.frameType, marker.action, event.role,
        ...
        const aggregate = yearEvidence.get(key) || { projectId: eventProjectId, year, eventIds: new Set<string>() };
