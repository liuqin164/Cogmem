...
function boundTitle(title: string): string {
  ...
  if (/[^\x00-\x7F]/.test(clean)) return clean.length > 32 ? `${clean.slice(0, 31)}…` : clean;
  const words = clean.split(/\s+/);
  return words.length > 12 ? words.slice(0, 12).join(' ') : clean;
}

function fallbackDisplayTitle(input: { userText: string; summaryText: string; topicPath?: string | null; episodeType?: string | null }): string {
  ...
  if (!title || /^episode[:\s-]*[0-9a-f-]+$/i.test(title)) return '未命名记忆事件';
  return title;
}

function fallbackSummary(input: { userText: string; summaryText: string; topicPath?: string | null }): string {
...
function inferTopicHints(text: string): string[] {
