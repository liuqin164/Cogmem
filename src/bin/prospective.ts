...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (args.command === 'list') {
      ...
      result = service.list({ projectId, statuses: args.values.get('status') as ProspectiveMemoryStatus[] | undefined });
    } else if (args.command === 'due') {
      if (!projectId) throw new Error('--project is required');
      result = service.listDue({ projectId, atTime: numberValue(args.values, 'at') });
    } else if (args.command === 'create') {
      const candidateType = args.values.get('type')?.[0] as ProspectiveMemoryType | undefined;
      ...
      result = service.propose({
        ...
        proposedBy: (args.values.get('proposed-by')?.[0] as 'deterministic' | 'model_candidate' | 'operator') ?? 'operator',
      ...
      if (args.command === 'confirm') {
        ...
        if (!evidence) throw new Error('confirm requires --evidence <userEventId>');
        result = service.resolve(candidateId, { action: 'confirm', confirmationEvidenceEventId: evidence }, projectId);
      } else if (args.command === 'reject') result = service.resolve(candidateId, { action: 'reject' }, projectId);
      else if (args.command === 'complete') result = service.resolve(candidateId, { action: 'complete' }, projectId);
      else if (args.command === 'expire') result = service.resolve(candidateId, { action: 'expire' }, projectId);
      else if (args.command === 'defer') {
        const deferredUntil = numberValue(args.values, 'until');
        if (deferredUntil === undefined) throw new Error('defer requires --until <epoch-ms>');
