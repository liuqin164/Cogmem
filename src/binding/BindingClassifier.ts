...
const PROJECT_ALIASES: ProjectAlias[] = [
...
export class BindingClassifier {
  classify(text: string): BindingTopicDecision[] {
    ...
    if (!isHighValueText(normalized)) return [];

    const projects = detectProjects(normalized);
    const lowered = normalized.toLowerCase();
    const decisions: BindingTopicDecision[] = [];
...
function isHighValueText(text: string): boolean {
  const lowered = text.toLowerCase();
  if (text.length < 8) return false;
  if (/^(ok|okay|yes|no|thanks|thank you|continue|go on)$/i.test(text)) return false;
  if (/^(好的|好|嗯|继续|谢谢|可以|收到|明白|是的|不是)$/.test(text)) return false;
  return [
    'remember',
    'prefer',
    'must',
...
function detectProjects(text: string): ProjectAlias[] {
  const lowered = text.toLowerCase();
  const detected: ProjectAlias[] = [];
  ...
  if (
    !detected.some((project) => project.canonical === 'Cogmem')
...
function genericProjectTopic(
  text: string,
  projectName: string,
): { suffix: string; summary: string; signal: string; bindingType: MemoryBindingType; confidence: number } | undefined {
  const lowered = text.toLowerCase();
...
function hasMemoryWritePipelineCue(lowered: string): boolean {
...
function bindingTypeFor(text: string, fallback: MemoryBindingType): MemoryBindingType {
  const lowered = text.toLowerCase();
  if (/纠正|不对|错了|更正|correct|wrong/.test(lowered)) return 'correction';
  if (/不要|不能|禁止|必须|边界|must not|never|forbid|boundary/.test(lowered)) return 'boundary';
  if (/决定|方案|策略|decision|decide/.test(lowered)) return 'decision';
  if (/请记住|以后|偏好|prefer|local-first|本地优先/.test(lowered)) return 'preference';
  if (/目标|希望|goal|want/.test(lowered)) return 'goal';
  if (/问题|风险|不像|表格|漂移|problem|risk|diagnos/.test(lowered)) return 'diagnostic';
  return fallback;
}

function signalForClaim(text: string, fallback: string): string {
  const lowered = text.toLowerCase();
  if (/分类树|分类.*漂移|topic.*drift|tree.*drift|memory-storage|memory-write/.test(lowered)) return 'classification_drift';
  if (/历史|关联|binding|旧记忆|old memory|table|表格|孤立/.test(lowered)) return 'historical_binding';
  if (/污染|上下文|recall_context|turn_bridge|session_state/.test(lowered)) return 'context_pollution_boundary';
  return fallback;
}
