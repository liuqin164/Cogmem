...
import type { MemoryBindingInput, MemoryBindingRecord, MemoryBindingType, MemoryEntityType } from './MemoryBindingTypes.js';
...
export class MemoryBindingService {
  ...
  bindEvent(input: MemoryBindingEventInput): MemoryBindingRecord[] {
    ...
    if (!isHighValueText(text)) return [];
...
function classifyTopics(text: string): TopicDecision[] {
  const lowered = text.toLowerCase();
...
function bindingTypeFor(text: string, fallback: MemoryBindingType): MemoryBindingType {
  const lowered = text.toLowerCase();
  if (/不要|不能|禁止|边界|must not|never|forbid|boundary/.test(lowered)) return 'boundary';
  ...
  if (/问题|风险|不像|表格|problem|risk|diagnos/.test(lowered)) return 'diagnostic';
  return fallback;
}

function uniqueTopicDecisions(decisions: TopicDecision[]): TopicDecision[] {
