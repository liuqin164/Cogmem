...
export function classifyTurnRelation(input: string | TurnClassificationContext): TurnRelationDecision {
  ...
  return decision(
    'continues_previous', highValueAmbiguous ? 0.68 : 0.62, inferEpisodeType(text), inferImportance(text),
    'bounded_session_continuity', highValueAmbiguous ? ['high_value_ambiguous'] : ['session_continuity'], false,
    candidateTypes, highValueAmbiguous,
  );
}
...
function classifySwitch(text: string, active: string): 'hard' | 'ambiguous' {
...
function sameDomain(text: string, active: string): boolean {
  const nextDomain = domainOf(text);
  const activeDomain = domainOf(active);
...
function domainOf(text: string): string | undefined {
  if (MEMORY_DOMAIN.test(text)) return 'memory';
  if (CAR_DOMAIN.test(text)) return 'car';
  if (HEALTH_DOMAIN.test(text)) return 'health';
  if (TRANSLATION_DOMAIN.test(text)) return 'translation';
  return undefined;
}
