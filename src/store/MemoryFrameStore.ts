...
export class MemoryFrameStore {
  ...
  review(frameId: string, projectId: string, action: 'approve' | 'reject', actor: string, reason: string, now = Date.now()): boolean {
    ...
    if (!row || row.project_id !== projectId || row.status !== 'needs_confirmation') return false;
  ...
  supersedeEpisodes(episodeIds: string[], now = Date.now()): number {
    ...
    let changed = 0; let derivedChanged = false; for (const id of episodeIds) {
      ...
      if (this.tableExists('memory_atlas_supports')) {
        ...
        if (this.tableExists('memory_edges')) derivedChanged = Number(this.db.prepare(`UPDATE memory_edges SET status='archived', updated_at=? WHERE edge_id IN (SELECT node_id FROM memory_atlas_supports WHERE source_episode_id=? AND source_type='frame_edge') AND status IN ('active','weak') AND NOT EXISTS (SELECT 1 FROM memory_atlas_supports s WHERE s.node_id=memory_edges.edge_id AND s.source_type='frame_edge' AND s.status='active')`).run(now, id).changes ?? 0) > 0 || derivedChanged;
    ...
    if (changed || derivedChanged) for (const id of episodeIds) { const row = this.db.prepare(`SELECT project_id FROM memory_frames WHERE episode_id=? LIMIT 1`).get(id) as { project_id?: string } | null; if (row?.project_id) this.markDirty(row.project_id, now); }
    return changed;
    })();
  }
