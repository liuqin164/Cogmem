...
export function policyExecutionStateIsValid(
  status: PolicyExecutionStatus,
  outcome?: PolicyExecutionOutcome,
): boolean {
  if (status === 'in_progress') return outcome === undefined;
  if (status === 'executed' || status === 'skipped') return outcome === 'executed';
...
export class PolicyExecutionStore {
  ...
  private initializeSchema(): void {
    ...
    for (const [table, columns] of [
      ...
      if (!exists) continue;
      const actual = new Set((this.db.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>).map((row) => row.name));
      if (columns.some((column) => !actual.has(column))) throw new Error(`policy_execution_schema_not_migrated:${table}`);
    }
    this.db.exec(`
  ...
  finishClaim(record: PolicyExecutionRecord, leaseOwner: string, options?: { emitEvent?: boolean }): void {
    ...
    if (!policyExecutionStateIsValid(record.status, record.executionOutcome)) throw new Error('invalid_policy_execution_state');
    this.db.transaction(() => {
      ...
      `).run(
        ...
        record.status, record.executionOutcome ?? null, record.attemptCount, record.nextRetryAt ?? null,
  ...
  flushAuditOutbox(): number {
    ...
    for (;;) {
      ...
      `).all(dueAt) as Array<{ outbox_id: string; payload_json: string; attempt_count: number }>;
      if (rows.length === 0) break;
      for (const row of rows) {
        try {
          const record = JSON.parse(row.payload_json) as PolicyExecutionRecord;
          if (!this.eventStore.getEvent(row.outbox_id)) this.emitRecord(record, row.outbox_id);
          ...
          `).run(
            ...
            malformed || attempts >= 5 ? null : failedAt + Math.min(60_000, 1000 * 2 ** (attempts - 1)),
            malformed || attempts >= 5 ? failedAt : null,
            row.outbox_id,
          );
        }
      }
  ...
  getAuditOutboxStats(): { pending: number; oldestCreatedAt?: number; deadLetter: number; lastError?: string } {
    const row = this.db.prepare(`
    ...
    `).get() as { pending: number | null; oldest_created_at: number | null; dead_letter: number | null };
  ...
  upsertReadModel(record: PolicyExecutionRecord, sourceGlobalSeq = 0, staging = false): void {
    ...
    `).run(
      ...
      record.policy, record.action, record.target ?? null, record.status, record.executionOutcome ?? null,
      record.attemptCount,
