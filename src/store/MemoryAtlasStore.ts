...
interface AtlasDocumentRow {
  node_id: string; project_id: string; node_type: string; source_id: string; label: string;
...
export class MemoryAtlasStore {
  ...
  upsertDocument(input: Omit<MemoryAtlasNode, 'activation' | 'score' | 'evidenceCount'> & { evidenceEventIds?: string[]; metadata?: Record<string, unknown>; updatedAt?: number }): void {
    this.db.prepare(`
    ...
    `).run(input.id, input.projectId, input.nodeType, input.sourceId, input.label, input.summary || null,
  ...
  searchFaceted(query: string, projectId: string, limit: number, facets: { from?: number; to?: number; memoryKinds?: string[] }): MemoryAtlasNode[] {
    const tokens = (query.match(/[\p{L}\p{N}_-]+/gu) || []).filter((item) => item.length > 1).slice(0, 12);
    if (!tokens.length) return this.listNodes(projectId, limit);
    const clauses = tokens.map(() => `(d.label LIKE ? ESCAPE '\\' OR COALESCE(d.summary,'') LIKE ? ESCAPE '\\' OR COALESCE(d.topic_path,'') LIKE ? ESCAPE '\\')`);
    const likeParams: Array<string | number> = tokens.flatMap((token) => [`%${escapeLike(token)}%`, `%${escapeLike(token)}%`, `%${escapeLike(token)}%`]);
    ...
    if (facets.memoryKinds?.length) {
      ...
      for (const kind of facets.memoryKinds) facetParams.push(kind, `%${escapeLike(kind)}%`, `%${escapeLike(kind)}%`, `%${escapeLike(kind)}%`);
    }
    ...
    let rows = this.db.prepare(`
      ...
      ORDER BY COALESCE(a.activation,0) DESC, d.support_count DESC, d.updated_at DESC LIMIT ?
    `).all(ftsQuery, projectId, ...facetParams, limit) as AtlasDocumentRow[];
    if (!rows.length) rows = this.db.prepare(`
      SELECT d.*, COALESCE(a.activation, 0) AS activation
      FROM memory_atlas_documents d LEFT JOIN memory_atlas_activation a
        ON a.project_id=d.project_id AND a.node_id=d.node_id
      WHERE d.project_id=? AND (${clauses.join(' OR ')}) ${filterSql} AND d.status NOT IN ('rejected','archived')
      ORDER BY COALESCE(a.activation,0) DESC, d.support_count DESC, d.updated_at DESC LIMIT ?
    `).all(projectId, ...likeParams, ...facetParams, limit) as AtlasDocumentRow[];
    return rows.map((row) => {
  ...
  listActions(projectId: string, options: { target?: string; from?: number; to?: number; limit: number }): MemoryAtlasAction[] {
    const clauses = ['project_id=?']; const params: Array<string | number> = [projectId];
    if (options.target) { clauses.push(`target_label LIKE ? ESCAPE '\\'`); params.push(`%${escapeLike(options.target)}%`); }
  ...
  recordAccess(projectId: string, nodeIds: string[], kind: string, query?: string): void {
    const now = Date.now(); const hash = query ? createHash('sha256').update(query).digest('hex') : null;
