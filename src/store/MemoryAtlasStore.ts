...
export class MemoryAtlasStore {
  ...
  resolveTargetNodeIds(projectId: string, query: string): { nodeIds: string[]; entitySourceIds: string[]; labels: string[] } {
    ...
    try {
      ...
      `).all(projectId, normalizedQuery) as Array<Record<string, unknown>>;
      for (const row of aliasRows) if (!seeds.some((seed) => seed.node_id === row.node_id)) { seeds.push(row); labels.push(String(row.label)); }
