...
export class ContextCortex {
  ...
  classifyIntent(query: string): ContextIntent {
    ...
    if (/为什么.*(改|变|推翻)|何时|什么时候|之前.*决定|decision.*(change|history)|why.*change|timeline|历史/.test(text)) return 'decision_history';
    if (/偏好|喜欢|不喜欢|边界|习惯|preference|prefer|boundary/.test(text)) return 'preference_lookup';
    if (/项目.*(状态|进度|当前)|release status|project status|current state/.test(text)) return 'project_status';
    if (/报错|错误|调试|根因|bug|debug|error|failure|root cause/.test(text)) return 'debugging';
  ...
  plan(input: ContextPlanInput): ContextActivationPlan {
    ...
    const eligible: Array<{ candidate: ContextCandidate; tokens: number }> = [];
  ...
  getReceipt(receiptId: string): ContextActivationReceipt | null {
    ...
    const row = this.db.prepare(`SELECT receipt_json FROM context_activation_receipts WHERE receipt_id = ?`).get(receiptId) as { receipt_json: string } | null;
  ...
  private hardSuppressionReason(candidate: ContextCandidate, input: ContextPlanInput, intent: ContextIntent): ContextSuppressionReason | undefined {
    ...
    if (candidate.ownership === 'user' && !candidate.sourceRoles?.includes('user')) return 'user_belief_without_user_evidence';
    if (candidate.sensitive && !input.allowSensitive && intent !== 'exact_quote') return 'sensitive_without_need';
    return undefined;
  }

  private estimateTokens(candidate: ContextCandidate): number {
  ...
  private persistReceipt(receipt: ContextActivationReceipt): void {
    if (!this.db) return;
    this.db.prepare(`
