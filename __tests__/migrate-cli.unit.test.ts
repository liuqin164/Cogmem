...
test('unsafe migration receipts fail before creating a misleading pre-migration backup', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'cogmem-unsafe-preflight-'));
  const dbPath = join(dir, 'memory.db');
  const db = new Database(dbPath);
  db.exec(`CREATE TABLE _schema_migrations(
    version TEXT PRIMARY KEY,description TEXT NOT NULL,applied_at TEXT NOT NULL,checksum TEXT
  )`);
  db.prepare(`INSERT INTO _schema_migrations VALUES(?,?,?,?)`).run(
    '0059',
    'unsafe temporary migration',
  ...
  expect(result.stderr).toContain('migration_recovery_required:0059');
  expect(readdirSync(dir).filter((name) => name.includes('pre-migrate') || name.endsWith('.bak'))).toEqual([]);
...
test('cogmem migrate plans and upgrades a 2.7.1 database with a backup', async () => {
  ...
  expect(dryRun.exitCode).toBe(0);
    expect(JSON.parse(dryRun.stdout).pending).toEqual(['0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036', '0037', '0038', '0039', '0040', '0041', '0042', '0043', '0044', '0045', '0046', '0047', '0048', '0049', '0050', '0051', '0052', '0053', '0054', '0055', '0056', '0057', '0058', '0059', '0060', '0061', '0062']);
  ...
  const result = JSON.parse(applied.stdout);
    expect(result.applied).toEqual(['0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036', '0037', '0038', '0039', '0040', '0041', '0042', '0043', '0044', '0045', '0046', '0047', '0048', '0049', '0050', '0051', '0052', '0053', '0054', '0055', '0056', '0057', '0058', '0059', '0060', '0061', '0062']);
  ...
  expect(migrated.prepare(`SELECT value FROM _meta WHERE key = 'schema_version'`).get()).toEqual({ value: '62' });
...
test('current kernel databases remain idempotent through source and dist migrate commands', async () => {
  for (const target of [migrateBin, distMigrateBin]) {
    const dir = mkdtempSync(join(tmpdir(), 'cogmem-current-migrate-'));
    const dbPath = join(dir, 'memory.db');
    ...
    expect(db.prepare(`SELECT COUNT(*) AS count FROM _schema_migrations WHERE checksum IS NULL OR checksum=''`).get())
      .toEqual({ count: 0 });
    db.close();
    rmSync(dir, { recursive: true, force: true });
  }
...
test('real f71b20a source and dist databases upgrade through current source and dist', async () => {
  ...
  for (const fixture of fixtures) {
    for (const target of targets) {
      ...
      expect(JSON.parse(upgraded.stdout).applied).toEqual(['0046', '0047', '0048', '0049', '0050', '0051', '0052', '0053', '0054', '0055', '0056', '0057', '0058', '0059', '0060', '0061', '0062']);

      const db = new Database(dbPath, { readonly: true });
      expect(db.prepare(`SELECT MAX(version) AS version FROM _schema_migrations`).get()).toEqual({ version: '0062' });
