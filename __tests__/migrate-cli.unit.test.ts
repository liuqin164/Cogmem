...
test('cogmem migrate plans and upgrades a 2.7.1 database with a backup', async () => {
  ...
  expect(dryRun.exitCode).toBe(0);
    expect(JSON.parse(dryRun.stdout).pending).toEqual(['0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036', '0037', '0038', '0039', '0040', '0041', '0042', '0043', '0044', '0045', '0046', '0047', '0048', '0049', '0050', '0051', '0052']);
  ...
  const result = JSON.parse(applied.stdout);
    expect(result.applied).toEqual(['0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036', '0037', '0038', '0039', '0040', '0041', '0042', '0043', '0044', '0045', '0046', '0047', '0048', '0049', '0050', '0051', '0052']);
  ...
  expect(migrated.prepare(`SELECT value FROM _meta WHERE key = 'schema_version'`).get()).toEqual({ value: '52' });
...
test('real f71b20a source and dist databases upgrade through current source and dist', async () => {
  ...
  for (const fixture of fixtures) {
    for (const target of targets) {
      ...
      expect(JSON.parse(upgraded.stdout).applied).toEqual(['0046', '0047', '0048', '0049', '0050', '0051', '0052']);

      const db = new Database(dbPath, { readonly: true });
      expect(db.prepare(`SELECT MAX(version) AS version FROM _schema_migrations`).get()).toEqual({ version: '0052' });
