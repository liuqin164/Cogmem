...
test('cogmem migrate plans and upgrades a 2.7.1 database with a backup', async () => {
  ...
  expect(JSON.parse(dryRun.stdout).pending).toEqual(['0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025']);

  const applied = await run(['--db', dbPath, '--yes', '--backup', '--json']);
  expect(applied.exitCode).toBe(0);
  const result = JSON.parse(applied.stdout);
  expect(result.applied).toEqual(['0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025']);
  ...
  expect(migrated.prepare(`SELECT value FROM _meta WHERE key = 'schema_version'`).get()).toEqual({ value: '25' });
