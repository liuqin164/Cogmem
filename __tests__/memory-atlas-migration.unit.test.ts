...
test('one command upgrades a 3.5.2 database through schema 35 without changing source memory', async () => {
  ...
  fixture.exec(`
    ...
    DELETE FROM _schema_migrations WHERE version IN ('0025','0026','0027','0028','0029','0030','0031','0032','0033','0034','0035','0036');
  ...
  expect(dryRun.pending).toEqual(['0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036']);

  const result = await migrate(dbPath, ['--yes', '--backup']);
  expect(result.applied).toEqual(['0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036']);
  expect(existsSync(result.backupPath as string)).toBe(true);

  const upgraded = new Database(dbPath, { readonly: true });
  expect(upgraded.prepare(`SELECT value FROM _meta WHERE key = 'schema_version'`).get()).toEqual({ value: '36' });
