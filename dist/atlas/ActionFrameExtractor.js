...
export class ActionFrameExtractor {
    ...
    rebuild(projectId) {
        ...
        const bindings = this.db.prepare(`SELECT b.* FROM memory_bindings b ${where} ORDER BY b.created_at`).all(...(projectId ? [projectId] : []));
        let created = 0;
        for (const binding of bindings) {
            if (binding.role !== 'user' || !binding.entity_id || !binding.project_id)
                continue;
            const event = this.eventStore.getEvent(String(binding.event_id));
            if (!event || event.projectId !== String(binding.project_id))
                continue;
            ...
            const episode = this.db.prepare(`SELECT episode_id FROM memory_episode_events WHERE event_id=? LIMIT 1`).get(eventId);
