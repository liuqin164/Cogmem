...
function main() {
    ...
    try {
        ...
        if (apply)
            throw new Error('Refusing project-scope repair: generic project_id rewrites are unsafe; use a schema migration or restore/import into the intended project.');
        const changed = 0;
        const payload = {
...
function repairTaskIdentity() {
    ...
    if (!['active', 'derived'].includes(status))
        throw new Error('Invalid task status; expected active or derived.');
    const apply = hasFlag('--apply');
    const dbPath = dbPathFromArgs();
    const db = new Database(dbPath);
    db.exec('PRAGMA busy_timeout = 5000;');
    try {
        ...
        if (recovery?.recovery_status !== 'unresolved' || !quarantine)
            throw new Error(`Task is not unresolved or lacks quarantine provenance: ${taskId}`);
        if (projectId !== current.project_id)
            throw new Error('Task identity repair cannot change project scope.');
        ...
        if (collision)
            throw new Error(`Task identity collision: ${projectId}:${taskKey}`);
        if (apply)
            db.transaction(() => {
        ...
        VALUES(?,?,?,?,?)`).run(`task-identity-${taskId}-${Date.now()}`, '0058', 'task_identity_operator_confirmation', JSON.stringify({ taskId, projectId, previousTaskKey: oldKey, taskKey, title, status }), Date.now());
            })();
