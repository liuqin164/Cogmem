...
export function extractEntityCues(text, limit = 8) {
    ...
    for (const label of labels.map(cleanEntityLabel).filter(Boolean)) {
        ...
        if (!id || ENTITY_STOP_WORDS.has(id) || seen.has(id))
            continue;
        seen.add(id);
        cues.push({ label, id });
...
export function inferOperationalActionCue(text) {
    if (/启动|start|started|launch|launched|boot/i.test(text))
        return { kind: 'started', label: '启动', verb: '启动' };
    if (/安装|install|installed|setup/i.test(text))
        return { kind: 'installed', label: '安装', verb: '安装' };
    if (/配置|config|configured|设置|修改配置|修改/i.test(text))
        return { kind: 'configured', label: '配置', verb: '配置' };
    if (/重启|restart|restarted/i.test(text))
        return { kind: 'restarted', label: '重启', verb: '重启' };
    if (/停止|stop|stopped/i.test(text))
        return { kind: 'stopped', label: '停止', verb: '停止' };
    if (/操作|处理|执行|运行|run|ran/i.test(text))
        return { kind: 'operated', label: '操作', verb: '操作' };
    return undefined;
}
