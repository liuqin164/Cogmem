...
export class MemoryKernel {
    ...
    rebuildProjectTimeTopology(projectId) {
        ...
        const now = Date.now();
        const db = this.factStore.getDatabase();
        let job = db.prepare(`SELECT generation,time_zone,status,cursor_created_at,cursor_neuron_id,neuron_count FROM topology_time_rebuild_jobs WHERE project_id=?`).get(projectId);
                ...
                db.transaction(() => {
                    ...
                    for (;;) {
                        ...
                        if (neurons.length === 0)
                            break;
                        for (const neuron of neurons)
                            insertActive.run(generation, neuron.id, projectId, neuron.createdAt, neuron.title);
                        neuronCount += neurons.length;
                        const last = neurons[neurons.length - 1];
