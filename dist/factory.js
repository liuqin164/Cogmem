...
export class MemoryKernel {
    ...
    constructor(options = {}) {
      ...
      `).get());
        ...
        if (!existingSchema)
            new SchemaMigrationRunner(db, KERNEL_MIGRATIONS, { backupVerified: true }).run();
    ...
    publishTimeProjectionAtomically(input) {
        const db = this.factStore.getDatabase();
        db.transaction(() => {
            const now = Date.now();
    ...
    async consolidate(options = {}) {
        ...
        ORDER BY project_id
      `).all(startTime, endTime).map((row) => row.project_id);
            const outputs = [];
            for (const projectId of scopes)
                outputs.push(await this.consolidate({ ...options, projectId, startTime, endTime }));
    ...
    async forgetUser(projectId, _reason = 'unspecified') {
        ...
        const mentionScopes = new Map();
        for (const row of db.prepare(`SELECT entity_id,COALESCE(project_id,'') AS scope FROM entity_mentions`).all()) {
            const scopes = mentionScopes.get(row.entity_id) ?? new Set();
    ...
    `).all()
            ...
            if (typeof ownerProjectId === 'string')
                return projectScope(ownerProjectId) === scope;
            const scopes = mentionScopes.get(row.instance_id) ?? new Set();
      ...
      `).all(...projectMentionEntityIds).map((row) => row.canonical_entity_id)),
        ]);
        ...
        const runDelete = (sql, params = []) => {
            try {
                ...
                if (error instanceof Error) {
                    ...
                    if (missing && target && missing.toLowerCase() === target.toLowerCase())
                        return 0;
                }
                throw error;
            }
        ...
        db.transaction(() => {
            Object.assign(deleted, { privacyTables: deleteRegisteredProjectContent({
                    ...
                    neuronIds,
                    runDelete,
                    listPersistentTables: () => db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'`).all().map((row) => row.name),
      ...
      `).all(scope, scope).map((row) => row.entity_id);
            ...
            Object.assign(deleted.privacyTables ??= {}, residualPrivacyTables);
            if (neuronIds.length > 0) {
...
function rebuildCanonicalEntityAfterForget(db, canonicalEntityId, runDelete) {
  ...
  ) ORDER BY updated_at DESC,alias_id`).all(canonicalEntityId);
