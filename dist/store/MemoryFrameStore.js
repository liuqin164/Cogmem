            ...
            for (const id of episodeIds) {
                ...
                if (this.tableExists('memory_atlas_supports')) {
                    ...
                    if (this.tableExists('memory_edges'))
                        derivedChanged = Number(this.db.prepare(`UPDATE memory_edges SET status='archived', updated_at=? WHERE edge_id IN (SELECT node_id FROM memory_atlas_supports WHERE source_episode_id=? AND source_type='frame_edge') AND status IN ('active','weak') AND NOT EXISTS (SELECT 1 FROM memory_atlas_supports s WHERE s.node_id=memory_edges.edge_id AND s.source_type='frame_edge' AND s.status='active')`).run(now, id).changes ?? 0) > 0 || derivedChanged;
