...
export class MemoryFrameStore {
    ...
    list(projectId, options = {}) {
        ...
        const rows = this.db.prepare(`SELECT * FROM memory_frames WHERE project_id=? AND status IN (${placeholders}) ORDER BY updated_at DESC, frame_id DESC LIMIT ?`).all(projectId, ...statuses, Math.max(1, Math.min(options.limit ?? 100, 1000)));
