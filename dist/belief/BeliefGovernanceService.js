...
export class BeliefGovernanceService {
    ...
    apply(input) {
        ...
        if (!canonicalKey || !statement)
            throw new Error('belief_key_and_statement_required');
        ...
        if (current && (relation === 'reinforce' || (relation === 'assert' && sameStatement))) {
            return this.reinforce(current, input, evidence);
        }
        const status = current && relation !== 'correct'
            ? 'possible_conflict'
            : 'active';
      ...
      `).run(beliefId, input.projectId ?? null, input.ownership, input.beliefType, canonicalKey, statement, status, confidence, now, relation === 'correct' ? current?.beliefId ?? null : null, now, now);
            this.insertEvidence(beliefId, evidence, relation === 'contradict' ? 'contradicts' : 'supports', now);
    ...
    getCurrent(projectId, canonicalKey) {
    ...
    getHistory(projectId, canonicalKey) {
        const rows = this.db.prepare(`
      SELECT * FROM belief_graph_nodes
    ...
    reinforce(current, input, evidence) {
        const now = input.occurredAt ?? Date.now();
    ...
    insertEvidence(beliefId, evidence, evidenceType, now) {
        const statement = this.db.prepare(`
      INSERT OR IGNORE INTO belief_graph_evidence (belief_id, event_id, source_role, evidence_type, weight, created_at)
      VALUES (?, ?, ?, ?, 1.0, ?)
    `);
        for (const item of evidence)
            statement.run(beliefId, item.eventId, item.role ?? 'unknown', evidenceType, now);
    }
    insertVersion(beliefId, version, reason, eventId, now) {
        const row = this.db.prepare(`SELECT * FROM belief_graph_nodes WHERE belief_id = ?`).get(beliefId);
        this.db.prepare(`
