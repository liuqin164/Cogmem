...
const PROSPECTIVE_MEMORY_TYPES = ['intention', 'commitment', 'reminder', 'open_loop', 'plan'];
const PROSPECTIVE_MEMORY_STATUSES = ['pending', 'confirmed', 'deferred', 'rejected', 'completed', 'expired'];
const PROSPECTIVE_MEMORY_PROPOSERS = ['deterministic', 'model_candidate', 'operator'];
const TERMINAL_PROSPECTIVE_MEMORY_STATUSES = ['rejected', 'completed', 'expired'];
export class ProspectiveMemoryService {
    db;
    findEvidence;
    constructor(db, findEvidence) {
        this.db = db;
    ...
    proposeInTransaction(input) {
        const projectId = requireProjectId(input.projectId);
        if (!PROSPECTIVE_MEMORY_TYPES.includes(input.candidateType))
            throw new Error('invalid_prospective_memory_type');
        if (!PROSPECTIVE_MEMORY_PROPOSERS.includes(input.proposedBy))
            throw new Error('invalid_prospective_memory_proposer');
        const canonicalKey = input.canonicalKey.trim().toLowerCase();
        const title = input.title.trim();
        if (!canonicalKey || !title)
            throw new Error('prospective_key_and_title_required');
        if (input.dueAt !== undefined && (!Number.isFinite(input.dueAt) || input.dueAt < 0))
        ...
        if (evidence.some((item) => item?.projectId !== projectId))
        ...
        if (latest && JSON.stringify([...latest.evidenceEventIds].sort()) === JSON.stringify(evidenceIds))
            return latest;
        const now = Date.now();
    ...
    `).run(candidateId, projectId, input.candidateType, canonicalKey, title, input.details ?? null, input.proposedBy, JSON.stringify(evidenceIds), input.dueAt ?? null, version, now, now);
    ...
    resolveInTransaction(candidateId, input, projectId) {
        ...
        switch (input.action) {
            case 'confirm': {
                ...
                if (!evidence)
                    throw new Error('unknown_confirmation_evidence');
                if (evidence.projectId !== current.projectId)
                    throw new Error('project_boundary_violation');
                if (evidence.role !== 'user')
                    throw new Error('confirmation_requires_user_evidence');
                ...
                const proposalSequences = proposalEvidence.map((item) => item?.globalSeq).filter((value) => Number.isFinite(value));
        ...
        `).get(current.projectId, input.confirmationEvidenceEventId, candidateId);
                if (priorUse)
                    throw new Error('confirmation_evidence_already_used');
                status = 'confirmed';
            ...
            case 'defer':
                if (current.status !== 'confirmed')
                    throw new Error('only_confirmed_memory_can_be_deferred');
                if (!Number.isFinite(input.deferredUntil) || input.deferredUntil <= now)
                    throw new Error('invalid_deferred_until');
                status = 'deferred';
                deferredUntil = input.deferredUntil;
                break;
            case 'complete':
                if (current.status !== 'confirmed')
                    throw new Error('only_confirmed_memory_can_be_completed');
                status = 'completed';
                break;
            case 'expire':
                status = 'expired';
    ...
    list(options) {
      ...
      LIMIT ?
    `).all(projectId, ...statuses, boundedLimit(options.limit, 100, 500));
    ...
    listDue(input) {
        ...
        if (!Number.isFinite(atTime) || atTime < 0)
            throw new Error('invalid_at_time');
        const rows = this.db.prepare(`
      SELECT * FROM prospective_memories
    ...
    `).all(projectId, atTime, atTime, boundedLimit(input.limit, 20, 100));
        return rows.map((row) => this.mapRow(row));
    }
    getLatest(projectId, canonicalKey) {
        const row = this.db.prepare(`
      SELECT * FROM prospective_memories WHERE project_id = ? AND canonical_key = ?
      ORDER BY version DESC LIMIT 1
    `).get(projectId, canonicalKey);
        return row ? this.mapRow(row) : null;
    }
    getUnscoped(candidateId) {
        const row = this.db.prepare(`SELECT * FROM prospective_memories WHERE candidate_id = ?`)
            .get(candidateId);
        return row ? this.mapRow(row) : null;
    }
    recordTransition(candidateId, from, to, action, eventId, now) {
        this.db.prepare(`
...
function isExplicitProspectiveConfirmation(content) {
