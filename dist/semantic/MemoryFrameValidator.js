...
export function validateMemoryFrame(value) {
    ...
    for (const reference of frame.temporalReferences) {
        if (!reference.label.trim() || !Number.isFinite(reference.confidence) || reference.confidence < 0 || reference.confidence > 1)
    ...
    for (const transition of frame.stateTransitions) {
        ...
        if (!transition.to.trim() || !Number.isFinite(transition.confidence) || transition.confidence < 0 || transition.confidence > 1)
