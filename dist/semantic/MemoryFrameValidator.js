...
export function validateMemoryFrame(value, options = {}) {
    ...
    if (frame.primaryLanguage !== undefined && typeof frame.primaryLanguage !== 'string')
    ...
    if (!frame.processor || typeof frame.processor.promptVersion !== 'string' || !frame.processor.promptVersion.trim() || !Number.isFinite(frame.processor.generatedAt) || (frame.processor.provider !== undefined && typeof frame.processor.provider !== 'string') || (frame.processor.model !== undefined && typeof frame.processor.model !== 'string'))
    ...
    for (const node of frame.nodes) {
        ...
        if (node.aliases !== undefined && (!Array.isArray(node.aliases) || node.aliases.length > limits.aliases || node.aliases.some((alias) => typeof alias !== 'string' || alias.length > 1000)))
    ...
    for (const transition of frame.stateTransitions) {
        ...
        if (typeof transition.to !== 'string' || (transition.from !== undefined && typeof transition.from !== 'string') || !Array.isArray(transition.evidenceEventIds) || !transition.to.trim() || (transition.from !== undefined && !transition.from.trim()) || !Number.isFinite(transition.confidence) || transition.confidence < 0 || transition.confidence > 1 || !transition.evidenceEventIds.length)
