...
export const migration_0062 = {
    ...
    `);
        ...
        if (tableExists(db, 'runtime_projection_state'))
            db.exec(`DELETE FROM runtime_projection_state`);
...
export function projectionScopeAndOutboxRecoverySatisfied(db) {
    try {
        assertProjectionScopeAndOutboxRecovery(db);
        return true;
    }
...
function renameDiscardReceipts(db) {
...
function recordUnscopedEvents(db) {
    if (!tableExists(db, 'memory_events'))
        return;
    const rows = db.prepare(`
    SELECT event_id,COALESCE(global_seq,0) AS global_seq,event_type
    FROM memory_events
  ...
  `);
    for (const row of rows) {
        ...
        const recordHash = createHash('sha256')
            .update(`${row.event_id}\0${row.global_seq}\0${row.event_type}`)
            .digest('hex');
        insert.run(projector, row.event_id, row.global_seq, row.event_type, recordHash, 'legacy_event_scope_unproven', Date.now());
    }
}
function reinstallIndexes(db) {
    for (const [name, table, columns] of INDEXES) {
        db.exec(`DROP INDEX IF EXISTS ${name}`);
        db.exec(`CREATE INDEX ${name} ON ${table}(${columns.join(',')})`);
    }
}
function assertProjectionScopeAndOutboxRecovery(db) {
    if (!tableExists(db, 'runtime_scope_discard_receipts'))
        throw new Error('runtime_scope_discard_receipts_missing');
    if (!tableExists(db, 'projection_event_discard_receipts'))
        throw new Error('projection_event_discard_receipts_missing');
    ...
    for (const [name, table, columns] of INDEXES) {
        ...
        if (!index || index.unique !== 0)
            throw new Error(`projection_index_mismatch:${name}`);
        const actual = db.prepare(`PRAGMA index_info(${name})`).all()
...
function addColumn(db, table, name, definition) {
    if (!tableExists(db, table) || tableInfo(db, table).some((column) => column.name === name))
        return;
...
function assertColumns(db, table, columns) {
    ...
    if (columns.some((column) => !actual.has(column)))
        throw new Error(`projection_columns_missing:${table}`);
}
