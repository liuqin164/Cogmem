...
function scrubMemoryEntityPayloads(db) {
    if (!tableExists(db, 'memory_entities'))
        return;
    ensureAuditTable(db);
    const rows = db.prepare(`SELECT entity_id,COALESCE(project_id,'') AS project_id FROM memory_entities`).all();
    let proven = 0;
