...
export function installAtlasProjectionDirtyTriggersV2(db) {
...
export function executionProjectionAndAtlasReliabilitySatisfied(db) {
    try {
        assertExecutionProjectionAndAtlasReliability(db);
        return true;
    }
...
function createReliabilitySchema(db) {
    db.exec(`
...
function assertExecutionProjectionAndAtlasReliability(db) {
    for (const table of [
        ...
        if (!tableExists(db, table))
            throw new Error(`execution_projection_table_missing:${table}`);
    }
    for (const table of ['policy_projection_state', 'runtime_projection_state', 'vector_projection_state']) {
        if (!tableColumns(db, table).has('last_global_seq'))
            throw new Error(`projection_global_seq_missing:${table}`);
    }
    if (!tableColumns(db, 'policy_execution_read_model').has('source_global_seq'))
        throw new Error('policy_read_model_sequence_missing');
    if (!tableColumns(db, 'runtime_projection_states').has('source_global_seq'))
        throw new Error('runtime_read_model_sequence_missing');
    assertAtlasTriggers(db);
}
function assertAtlasTriggers(db) {
    for (const trigger of atlasTriggerSpecs(db)) {
...
function atlasDirtyStatements(scope) {
    return `
