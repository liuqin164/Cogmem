...
function rebuildTimeTopologyIdentity(db) {
    const hadBuckets = tableExists(db, 'time_buckets');
    const bucketColumns = hadBuckets ? columns(db, 'time_buckets') : new Set();
    const buckets = hadBuckets
        ? db.prepare(`SELECT bucket_id,${bucketColumns.has('project_id') ? 'project_id' : 'NULL AS project_id'},${bucketColumns.has('time_zone') ? 'time_zone' : 'NULL AS time_zone'},bucket_type,bucket_start,bucket_end,label FROM time_buckets`).all()
        : [];
    const entries = tableExists(db, 'time_bucket_entries')
        ? db.prepare(`SELECT bucket_id,neuron_id,unit_id,belief_id,fact_id,event_id,project_id,created_at FROM time_bucket_entries`).all()
        : [];
    const timeZones = new Map();
    if (tableExists(db, 'topology_projection_state')) {
        for (const row of db.prepare(`SELECT project_id,time_zone FROM topology_projection_state WHERE time_zone IS NOT NULL AND time_zone<>''`).all()) {
  ...
  `);
    ...
    for (const entry of entries) {
        ...
        if (!scopedId)
            continue;
        insertEntry.run(scopedId, stringOrNull(entry.neuron_id), stringOrNull(entry.unit_id), stringOrNull(entry.belief_id), stringOrNull(entry.fact_id), stringOrNull(entry.event_id), projectId || null, Number(entry.created_at));
    }
    db.exec(`
...
function rebuildCognitiveGraphIdentity(db) {
    const nodes = tableExists(db, 'cognitive_nodes')
        ? db.prepare(`SELECT * FROM cognitive_nodes`).all()
        : [];
    const edges = tableExists(db, 'cognitive_edges')
        ? db.prepare(`SELECT * FROM cognitive_edges`).all()
        : [];
    const scopesByNode = new Map();
    for (const node of nodes)
        scopesByNode.set(String(node.node_id), new Set([String(node.project_id ?? '')]));
    for (const edge of edges) {
        const scope = String(edge.project_id ?? '');
        for (const nodeId of [String(edge.source_node_id), String(edge.target_node_id)]) {
            const scopes = scopesByNode.get(nodeId) ?? new Set();
            scopes.add(scope);
  ...
  `);
    ...
    for (const edge of edges) {
        ...
        if (edgeType === 'occurred_in_time_bucket')
            continue;
...
function validSourceNeuron(db, neuronId, projectId) {
    if (!neuronId || !tableExists(db, 'neurons'))
        return false;
