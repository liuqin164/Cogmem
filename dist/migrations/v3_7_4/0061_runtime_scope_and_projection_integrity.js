...
export function runtimeScopeAndProjectionIntegritySatisfied(db) {
    try {
        assertRuntimeScopeAndProjectionIntegrity(db);
        return true;
    }
...
export function installAtlasProjectionDirtyTriggersV3(db) {
    ...
    if (triggers.every((trigger) => triggerMatches(db, trigger)))
        return;
    const repair = () => {
    ...
    const inTransaction = Boolean(db.inTransaction);
    if (inTransaction)
        repair();
    else {
...
export function atlasProjectionDirtyTriggersV3Satisfied(db) {
    try {
        assertAtlasTriggersV3(db);
        return true;
    }
...
function repairTable(db, table) {
    if (!tableExists(db, table.name)) {
        db.exec(table.create);
        return;
    }
    if (tableSatisfied(db, table))
        return;
...
function quarantineRows(db, oldName, sourceTable, reason) {
    ...
    const rows = db.prepare(`SELECT * FROM ${oldName}`).all();
...
function assertRuntimeScopeAndProjectionIntegrity(db) {
    if (!tableExists(db, 'runtime_scope_quarantine'))
        throw new Error('runtime_scope_quarantine_missing');
    for (const table of TABLES) {
        if (!tableSatisfied(db, table))
            throw new Error(`projection_table_integrity_mismatch:${table.name}`);
    }
    assertAtlasTriggersV3(db);
    for (const index of [
...
function assertAtlasTriggersV3(db) {
    for (const trigger of atlasTriggerSpecs(db)) {
        if (!triggerMatches(db, trigger))
            throw new Error(`atlas_dirty_trigger_v3_mismatch:${trigger.name}`);
    }
}
function triggerMatches(db, trigger) {
...
function atlasTriggerSpecs(db) {
    ...
    return sources.flatMap((table) => ['INSERT', 'UPDATE', 'DELETE'].map((operation) => {
...
function atlasDirtyStatements(scope, guard = '1') {
    return `
...
function tableSatisfied(db, table) {
    const info = db.prepare(`PRAGMA table_info(${table.name})`).all();
    if (info.length !== table.columns.length)
        return false;
    if (info.map((column) => column.name).join('|') !== table.columns.join('|'))
        return false;
    if (table.notNull.some((name) => info.find((column) => column.name === name)?.notnull !== 1))
        return false;
