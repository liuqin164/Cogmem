...
export function topologyIntegritySatisfied(db) {
    try {
        assertTopologyIntegrity(db);
        return true;
    }
...
function repairBranches(db) {
    if (!tableExists(db, 'project_branches') || !tableExists(db, 'branch_entries'))
        return;
    const rows = db.prepare(`SELECT e.*,p.project_id FROM branch_entries e JOIN project_branches p ON p.branch_id=e.branch_id`).all();
...
function repairKind(db, kind) {
    ...
    const entries = db.prepare(`SELECT * FROM ${entryTable}`).all();
    ...
    for (const parent of parents) {
        ...
        for (const entry of byParent.get(oldId) ?? []) {
            ...
            if ('error' in resolved)
                quarantine(db, kind, oldId, parentScope, entry, resolved);
            else
                resolvedEntries.push({ entry: { ...entry, neuron_id: entry.neuron_id ?? resolved.neuronId }, resolved });
        }
        const scopes = new Set(resolvedEntries.map((item) => item.resolved.scope));
        if (resolvedEntries.length === 0 && (byParent.get(oldId)?.length ?? 0) === 0)
            addGroup(groups, kind, parentScope, key, parent, [], false);
        for (const item of resolvedEntries)
            addGroup(groups, kind, item.resolved.scope, key, parent, [item.entry], scopes.size > 1 || item.resolved.scope !== parentScope);
    }
    ...
    for (const group of groups.values()) {
        ...
        if (kind === 'task')
            insertParent.run(group.id, group.scope, group.key, title, group.entries.length > 0 ? 'derived' : String(group.parent.status ?? 'derived'), Number(group.parent.created_at), Number(group.parent.updated_at));
        else
            insertParent.run(group.id, group.scope, group.key, String(group.parent.cluster_type), title, Number(group.parent.created_at), Number(group.parent.updated_at));
        for (const entry of group.entries)
            insertEntry.run(group.id, group.scope, entry.neuron_id, entry.unit_id, entry.belief_id, entry.fact_id, entry.event_id, entry.created_at);
    }
...
function addGroup(groups, kind, scope, key, parent, entries, split) {
...
function resolveEntry(db, entry) {
    ...
    const addNeuron = (id) => {
        ...
        const row = tableExists(db, 'neurons') ? db.prepare(`SELECT COALESCE(project_id,'') AS scope,is_deleted FROM neurons WHERE id=?`).get(id) : null;
        if (!row) {
            error ||= 'entry_reference_dangling';
            return;
        }
            ...
            if (tableExists(db, 'belief_evidence'))
                for (const evidence of db.prepare(`SELECT neuron_id,event_id FROM belief_evidence WHERE belief_id=?`).all(entry.belief_id)) {
                    ...
                    if (evidence.event_id)
                        addEvidenceEventScope(db, evidence.event_id, scopes, addNeuron, () => { error ||= 'entry_reference_dangling'; });
                }
        }
    }
    ...
    if (error)
        return { error, implicatedScopes: distinct };
    return { scope: distinct[0], neuronId: neurons[0], implicatedScopes: distinct };
}
function addEvidenceEventScope(db, eventId, scopes, addNeuron, missing) {
    if (tableExists(db, 'memory_events')) {
        const row = db.prepare(`SELECT COALESCE(project_id,'') AS scope FROM memory_events WHERE event_id=?`).get(eventId);
        if (row) {
            scopes.push(row.scope);
            return;
        }
    ...
    if (tableExists(db, 'compiled_events')) {
        const row = db.prepare(`SELECT neuron_id FROM compiled_events WHERE event_id=?`).get(eventId);
        if (row) {
            addNeuron(row.neuron_id);
            return;
        }
...
function quarantine(db, kind, oldId, parentScope, entry, resolved) {
...
function rebuildCognitiveKind(db, kind) {
    ...
    for (const row of rows) {
        ...
        if (!source) {
            ...
            const neuron = db.prepare(`SELECT content,created_at FROM neurons WHERE id=? AND is_deleted=0`).get(row.neuron_id);
            if (!neuron)
                continue;
...
function assertTopologyIntegrity(db) {
    for (const [parentTable, entryTable, idColumn] of [['project_branches', 'branch_entries', 'branch_id'], ['task_branches', 'task_branch_entries', 'task_id'], ['event_clusters', 'event_cluster_entries', 'cluster_id']]) {
        if (!tableExists(db, parentTable) || !tableExists(db, entryTable))
            continue;
        const rows = db.prepare(`SELECT e.*,p.project_id AS parent_scope FROM ${entryTable} e LEFT JOIN ${parentTable} p ON p.${idColumn}=e.${idColumn}`).all();
        for (const row of rows) {
            const resolved = resolveEntry(db, row);
            if (row.parent_scope === null || 'error' in resolved || row.project_id !== row.parent_scope || resolved.scope !== row.parent_scope)
                throw new Error('topology_semantic_integrity_failed');
        }
    }
    if (tableExists(db, 'event_clusters') && db.prepare(`SELECT 1 FROM event_clusters WHERE cluster_key NOT LIKE cluster_type || ':%' LIMIT 1`).get())
        throw new Error('topology_cluster_key_noncanonical');
    if (tableExists(db, 'cognitive_edges') && tableExists(db, 'cognitive_nodes') && db.prepare(`SELECT 1 FROM cognitive_edges e LEFT JOIN cognitive_nodes s ON s.node_id=e.source_node_id LEFT JOIN cognitive_nodes t ON t.node_id=e.target_node_id WHERE s.node_id IS NULL OR t.node_id IS NULL OR e.project_id<>s.project_id OR e.project_id<>t.project_id LIMIT 1`).get())
        throw new Error('topology_cognitive_scope_mismatch');
    if (tableExists(db, 'topology_membership') && db.prepare(`SELECT 1 FROM topology_membership m JOIN neurons n ON n.id=m.neuron_id WHERE COALESCE(m.project_id,'')<>COALESCE(n.project_id,'') OR n.is_deleted<>0 LIMIT 1`).get())
        throw new Error('topology_membership_scope_mismatch');
}
function titleForGroup(db, kind, key, entries) {
    const id = entries.find((entry) => entry.neuron_id)?.neuron_id;
    if (id) {
        const row = db.prepare(`SELECT content FROM neurons WHERE id=? AND is_deleted=0`).get(id);
        if (row?.content)
            return row.content.slice(0, 96);
    }
    return `${kind}:${key}`;
}
function canonicalClusterKey(type, key) { return key.startsWith(`${type}:`) ? key : `${type}:${key}`; }
