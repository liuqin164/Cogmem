...
function rebuildIdentity(db, kind) {
    ...
    for (const parent of parents) {
        ...
        for (const entry of entries) {
          ...
          VALUES(?,?,?,?,?,?,?)`).run(stableId('quarantine', kind, oldId, JSON.stringify(entry)), kind, oldId, parentScope, JSON.stringify(entry), resolved.error, Date.now());
                continue;
            }
...
function inferEntryScope(db, entry) {
    ...
    if (deleted)
        return { error: 'entry_references_deleted_neuron' };
    const scopes = new Set(distinctIds.map((id) => scopeForNeuron(db, id)).filter((value) => value !== null));
    if (scopes.size > 1)
        return { error: 'entry_project_scope_conflict' };
    if (scopes.size === 0)
        return { error: 'entry_project_scope_unresolved' };
...
function scopeForNeuron(db, id) {
    if (!id || !tableExists(db, 'neurons'))
        return null;
    const row = db.prepare(`SELECT COALESCE(project_id,'') AS scope FROM neurons WHERE id=? AND is_deleted=0`).get(id);
    return row ? String(row.scope ?? '') : null;
}
function neuronIsDeleted(db, id) {
    if (!tableExists(db, 'neurons'))
        return false;
    return Boolean(db.prepare(`SELECT 1 FROM neurons WHERE id=? AND is_deleted<>0`).get(id));
}
function purgeDeletedCognitiveNodes(db) {
    if (!tableExists(db, 'neurons') || !tableExists(db, 'cognitive_nodes'))
        return;
