...
function isolateUnresolvedTasks(db) {
...
function repairBeliefIsolation(db) {
...
function repairSynapseIsolation(db) {
    ...
    WHERE source.id IS NULL OR target.id IS NULL
      OR COALESCE(source.project_id,'')<>COALESCE(target.project_id,'')`).all();
    for (const row of invalid)
        quarantine(db, 'synapse_0058', `${row.source_id}:${row.target_id}:${row.type}`, row, 'synapse_endpoint_scope_invalid');
...
function migratePendingEntityScope(db) {
    ...
    WHERE p.context_neuron_id IS NULL OR n.id IS NULL`).all();
    const insert = db.prepare(`INSERT OR REPLACE INTO pending_entity_resolution_quarantine VALUES(?,?,?,?)`);
    for (const row of invalid)
        insert.run(String(row.pending_id), JSON.stringify(row), 'pending_context_unproven', Date.now());
...
export function projectIsolationRuntimeGuardsSatisfied(db) {
    try {
        assertProjectIsolationRuntimeGuards(db);
        return true;
    }
...
function assertProjectIsolationRuntimeGuards(db) {
    if (tableExists(db, 'task_identity_restoration_manifest')) {
    ...
    if (tableExists(db, 'synapses')) {
        if (!tableColumns(db, 'synapses').has('project_id'))
            throw new Error('synapse_project_scope_missing');
      ...
      WHERE a.is_deleted<>0 OR b.is_deleted<>0 OR COALESCE(a.project_id,'')<>COALESCE(b.project_id,'')
        OR s.project_id<>COALESCE(a.project_id,'') LIMIT 1`).get())
            throw new Error('synapse_project_scope_invalid');
    }
    if (tableExists(db, 'pending_entity_resolution')
        && !tableColumns(db, 'pending_entity_resolution').has('project_scope'))
        throw new Error('pending_project_scope_missing');
}
function quarantine(db, type, id, row, reason) {
    if (!tableExists(db, 'topology_identity_quarantine'))
        return;
    const scope = String(row && typeof row === 'object' && 'project_id' in row ? row.project_id ?? '' : '');
  ...
  ) VALUES(?,?,?,?,?,?,?,?)`).run(`quarantine-${createHash('sha256').update(`${type}\0${id}`).digest('hex').slice(0, 32)}`, type, id, scope, JSON.stringify(row), reason, Date.now(), JSON.stringify([scope]));
}
