...
function preserveOrRejectTaskRecovery(db) {
...
function rebuildEntityRelations(db) {
...
function rebuildNeuronEmbeddings(db) {
    if (!tableExists(db, 'neuron_embeddings'))
        return;
...
function migrateMemoryEntityProjectionIds(db) {
    ...
    for (const row of rows) {
        ...
        if (tableExists(db, 'memory_bindings'))
            for (const item of db.prepare(`SELECT DISTINCT COALESCE(project_id,'') AS scope FROM memory_bindings WHERE entity_id=?`).all(oldId))
...
function entityScopes(db, entityId) {
    ...
    if (tableExists(db, 'entity_instances')) {
        const row = db.prepare(`SELECT json_extract(metadata_json,'$.projectId') AS project_id FROM entity_instances WHERE instance_id=?`).get(entityId);
        if (typeof row?.project_id === 'string')
            scopes.add(row.project_id);
    }
    if (tableExists(db, 'entity_mentions'))
        for (const row of db.prepare(`SELECT DISTINCT COALESCE(project_id,'') AS project_id FROM entity_mentions WHERE entity_id=?`).all(entityId))
            scopes.add(row.project_id);
    return [...scopes].sort();
}
