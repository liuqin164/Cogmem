const normalizeAlias = (value) => value.normalize('NFKC').toLocaleLowerCase('und').trim().replace(/\s+/gu, ' ');
const tableExists = (db, name) => Boolean(db.prepare(`SELECT 1 FROM sqlite_master WHERE type='table' AND name=?`).get(name));
const array = (value) => { try {
...
export const migration_0047 = {
    version: '0047',
    description: 'rebuild frame alias provenance from exact frame nodes',
    up(db) {
        if (!tableExists(db, 'memory_frames') || !tableExists(db, 'memory_atlas_alias_supports'))
            return;
        ...
        for (const frame of frames) {
            const nodes = db.prepare(`SELECT frame_node_id,dimension,label,aliases_json,canonical_hint_json,evidence_event_ids_json FROM memory_frame_nodes WHERE frame_id=?`).all(frame.frame_id);
            for (const node of nodes) {
                ...
                catch { /* invalid hint is handled by frame validation */ }
                const labels = [...new Set([node.label, ...array(node.aliases_json), ...(hintLabel ? [hintLabel] : [])].map(normalizeAlias).filter(Boolean))];
                for (const label of labels) {
                    const aliases = db.prepare(`SELECT alias_id,node_id,project_id FROM memory_atlas_aliases WHERE project_id=? AND dimension=? AND normalized_alias=? AND status='active'`).all(frame.project_id, node.dimension, label);
                    ...
                    if (matching.length === 1)
                        insert.run(`${matching[0].alias_id}:${frame.frame_id}`, matching[0].alias_id, frame.project_id, matching[0].node_id, frame.frame_id, frame.episode_id, node.evidence_event_ids_json ?? '[]', frame.created_at);
                    if (matching.length > 1 && tableExists(db, 'memory_atlas_alias_ambiguities'))
    ...
    `);
        if (db.prepare(`SELECT 1 FROM memory_atlas_alias_supports s JOIN memory_frames f ON f.frame_id=s.source_frame_id WHERE s.status='active' AND f.status<>'active' LIMIT 1`).get())
