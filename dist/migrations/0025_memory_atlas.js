...
export function backfillAtlasDocuments(db, projectId) {
    const now = Date.now();
    ...
    if (scoped)
        db.prepare(`DELETE FROM memory_atlas_fts WHERE project_id=?`).run(projectId);
    else
        db.exec(`DELETE FROM memory_atlas_fts;`);
...
function dropAtlasProjectionDirtyTriggers(db) {
    const rows = db.prepare(`SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE 'trg_memory_atlas_dirty_%'`).all();
    for (const row of rows) {
        if (!/^trg_memory_atlas_dirty_[a-z0-9_]+$/u.test(row.name))
            continue;
