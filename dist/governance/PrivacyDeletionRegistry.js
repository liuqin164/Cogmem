...
function explicitClassification(tables, groups) {
    const expected = new Set(tables);
    const result = {};
    for (const [classification, names] of Object.entries(groups)) {
        for (const name of names) {
            if (!expected.has(name))
                throw new Error(`privacy_schema_classification_extra:${name}`);
            if (result[name])
                throw new Error(`privacy_schema_classification_duplicate:${name}`);
            result[name] = classification;
        }
    }
    const missing = tables.filter((table) => !result[table]);
    if (missing.length > 0)
        throw new Error(`privacy_schema_classification_missing:${missing.join(',')}`);
...
const MIGRATION_TABLES = [
    ...
    'memory_atlas_alias_supports', 'memory_atlas_aliases', 'memory_atlas_documents', 'memory_atlas_projection_state', 'memory_atlas_supports', 'memory_bindings', 'memory_clusters', 'memory_edge_supports', 'memory_edges', 'memory_entities', 'memory_entity_scope_identity',
...
const MIGRATION_PROJECT_OWNED = [
    ...
    'memory_atlas_projection_state', 'memory_atlas_supports', 'memory_bindings', 'memory_clusters', 'memory_edge_supports', 'memory_edges', 'memory_entities', 'memory_entity_scope_identity',
...
export const PRIVACY_SCHEMA_CLASSIFICATION = Object.freeze({
...
export function deleteRegisteredProjectContent(context) {
    ...
    if (persistentTables.has('meta_observations')) {
        const clauses = [];
        const params = [];
        if (context.hasColumn('meta_observations', 'project_id')) {
            clauses.push(`COALESCE(project_id,'')=?`);
            params.push(scope);
        }
        if (context.hasColumn('meta_observations', 'neuron_id')) {
            clauses.push(`neuron_id IN (SELECT id FROM neurons WHERE COALESCE(project_id,'')=?)`);
            params.push(scope);
        }
        if (context.hasColumn('meta_observations', 'fact_id')) {
            clauses.push(`fact_id IN (SELECT fact_id FROM facts WHERE neuron_id IN (SELECT id FROM neurons WHERE COALESCE(project_id,'')=?))`);
            params.push(scope);
        }
        if (context.hasColumn('meta_observations', 'evidence_event_ids')) {
            ...
            params.push(scope);
            if (persistentTables.has('trace_events'))
                params.push(scope);
        }
        if (clauses.length > 0)
            remove('meta_observations', `DELETE FROM meta_observations WHERE ${clauses.join(' OR ')}`, params);
    }
    ...
    if (persistentTables.has('workspace_settings'))
        remove('workspace_settings', `DELETE FROM workspace_settings
    WHERE key=? OR key LIKE ? OR (json_valid(value) AND json_extract(value,'$.projectId')=?)`, [scope, `project:${scope}:%`, scope]);
    if (persistentTables.has('meta_proposals') && persistentTables.has('trace_events'))
        remove('meta_proposals', `DELETE FROM meta_proposals
    WHERE EXISTS (
    ...
    if (persistentTables.has('pending_entity_resolution_quarantine')) {
      ...
      WHERE (scope_resolved=1 AND project_scope=?)
        ...
        ${neuronIds.length > 0 ? `OR (json_valid(record_json) AND json_extract(record_json,'$.context_neuron_id') IN (${placeholders}))` : ''}`, [scope, scope, ...neuronIds]);
    }
...
export function deleteResidualProjectOwnedContent(context) {
    const audit = {};
    for (const table of context.listPersistentTables()) {
        if (table === 'neurons' || table === 'dream_ledger_state' || PRIVACY_SCHEMA_CLASSIFICATION[table] !== 'project_owned')
            continue;
...
export function assertPrivacySchemaClassified(tables) {
    const unknown = tables.filter((table) => {
        if (PRIVACY_SCHEMA_CLASSIFICATION[table])
            return false;
        return !/^(?:neurons|memory_events|memory_atlas|deep_write_summaries)_fts(?:_|$)/.test(table);
    });
    if (unknown.length > 0)
        throw new Error(`privacy_schema_unclassified:${unknown.sort().join(',')}`);
}
