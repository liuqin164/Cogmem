...
export class DreamScheduler {
    ...
    async tick(options = {}) {
        if (this.frameStore && this.frameStore.getDatabase() !== this.episodeStore.getDatabase())
            throw new Error('frame_store_database_mismatch');
        ...
        for (const [jobIndex, job] of jobs.entries()) {
            ...
            try {
                ...
                if (run.semanticProcessorAvailable)
                    semanticProcessorAvailable = true;
                if (run.semanticProcessorReason === 'semantic_processor_unavailable')
                    semanticProcessorUnavailableCount += 1;
                if (run.semanticProcessorReason === 'semantic_processor_failed')
                    semanticProcessorFailedCount += 1;
