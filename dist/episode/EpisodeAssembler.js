import { classifyAssistantRelation, classifyTurnRelation } from './TurnRelationClassifier.js';
export class EpisodeAssembler {
    store;
    resolveEvent;
    softReopenWindowMs;
    constructor(store, resolveEvent, softReopenWindowMs = 30 * 60_000) {
        this.store = store;
    ...
    classificationContext(primary, episode) {
        const context = {
    ...
    classifyPrimary(primary, episode) {
        if (primary.role === 'user')
            return classifyTurnRelation(this.classificationContext(primary, episode));
        return {
