...
export function classifyTurnRelation(text) {
    const input = String(text || '').trim();
    if (!input || NOISE.test(input))
        return decision('noise', 0.98, 'general', 0.05, 'deterministic_noise');
    if (TOPIC_SWITCH.test(input))
        return decision('switches_topic', 0.96, inferEpisodeType(input), inferImportance(input), 'explicit_topic_switch');
    if (CORRECTION.test(input))
        return decision('corrects_previous', 0.94, 'correction', 0.9, 'explicit_correction');
    if (CLOSURE.test(input))
        return decision('closes_episode', 0.92, inferEpisodeType(input), inferImportance(input), 'explicit_user_closure');
    if (CONTINUATION.test(input))
        return decision('continues_previous', 0.92, inferEpisodeType(input), inferImportance(input), 'explicit_continuation');
    if (PROSPECTIVE.test(input))
        return decision('confirms_future_intent', 0.82, 'prospective', 0.86, 'prospective_signal');
...
function decision(relation, confidence, episodeType, importance, rationale) {
